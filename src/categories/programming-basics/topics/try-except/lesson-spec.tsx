  ...
  return (
    <g>
      {revealed
        ...
        : <Cap>safe_divide(10, 2): 10 / 2 works &mdash; no error this run</Cap>}
  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>Programs meet a messy world: someone types <code>0</code>, a file has gone missing, the input is garbage. When a line fails like that, Python raises an <Term word="exception">error</Term> &mdash; announces the failure &mdash; and by default the whole program <strong>stops dead</strong>, right there.</>,
        }),
      }],
      detail: reg({
        ...
        intuitive: (
          <>
            <p>Picture a tiny calculator that divides two numbers a user typed in. The day someone types a zero, <code>10 / 0</code> has no answer &mdash; so Python raises a <code>ZeroDivisionError</code>: an alarm that says &ldquo;this line failed, and here&rsquo;s why.&rdquo;</p>
            <p>Left alone, that alarm doesn&rsquo;t just skip the line &mdash; it ends the entire run. One bad input, everything gone. So the question of this lesson: how do you <em>attempt</em> a risky line and stay standing when it fails?</p>
          </>
        ),
      }),
      codeLabels: ["def"],
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Put the line that might fail inside a <code>try:</code> block. Python will <strong>attempt</strong> it &mdash; and if anything goes wrong, instead of crashing it looks for a matching <code>except</code> to handle it.</>,
          intuitive: <>First move: draw a fence around the risky line. Putting it inside a <code>try:</code> <Term word="block">block</Term> tells Python &ldquo;<strong>attempt</strong> this &mdash; and if it blows up, don&rsquo;t die; go look for my backup plan instead.&rdquo;</>,
        }),
      }],
      detail: reg({
        ...
        intuitive: <><p>Note what <code>try</code> does <em>not</em> do: it doesn&rsquo;t fix anything, and on a good day it changes nothing at all. It only marks <em>where</em> trouble is allowed to happen &mdash; the fence Python watches while those lines run.</p></>,
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>The <code>except</code> block is the safety net. If the <code>try</code> raises that kind of error, Python <strong>jumps</strong> straight to <code>except</code> and runs it &mdash; here, returning <code>0</code> instead of crashing.</>,
          intuitive: <>Here&rsquo;s the backup plan. The moment the marked line fails, Python <strong>abandons it mid-step</strong> and leaps straight to the matching <code>except</code> block &mdash; here, one that calmly returns <code>0</code> instead of letting the program die.</>,
          rigorous: <>A raise inside the <code>try</code> aborts the block at that line; control transfers to the first <code>except</code> clause whose type matches (subclasses included). No match &mdash; the exception keeps propagating up the <Term word="call stack">call stack</Term>.</>,
        }),
      }],
      detail: reg({
        base: <><p>You name the error you expect (<code>ZeroDivisionError</code>) so you only catch what you understand. Catching <em>everything</em> blindly hides real bugs &mdash; be specific.</p></>,
        intuitive: (
          <>
            <p>Two things make the jump trustworthy. You <em>name</em> the trouble you expect &mdash; <code>except ZeroDivisionError:</code> only catches that one kind &mdash; so a surprise you <em>didn&rsquo;t</em> plan for still stops the program loudly instead of being quietly swallowed.</p>
        ...
        rigorous: (
          <>
            <p>The model: <code>try</code> delimits a watched region; a raise anywhere inside it transfers control to the first matching handler &mdash; matching is by exception type, subclass-aware, clauses tested top to bottom. The failed statement is never resumed. A raise the region doesn&rsquo;t cover (outside the <code>try</code>, or an unmatched type) propagates caller-by-caller until something matches &mdash; or the program dies with a traceback.</p>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>With <code>10</code> and <code>2</code>, <code>10 / 2</code> works fine. The <code>try</code> succeeds, returns <code>5.0</code>, and the <code>except</code> is <strong>skipped completely</strong> &mdash; it only exists for trouble.</>,
          intuitive: <>Hand it <code>10</code> and <code>2</code>: the division simply works. Two routes leave the <code>try</code> box &mdash; before the run lights one up, commit: what does the <code>except</code> block do when nothing fails?</>,
          rigorous: <>On <code>safe_divide(10, 2)</code> the block runs to its <code>return</code> with the handler never consulted. The construct&rsquo;s cost sits entirely on the raising path &mdash; predict, then confirm.</>,
        }),
      }],
      detail: reg({
        ...
        intuitive: (
          <>
            <p>Count it like a ledger. Call this function a thousand times today with good inputs: a thousand clean runs, and the <code>except</code> block never runs once &mdash; not even a peek. The protection costs you nothing on a good day; only the rare bad call will ever pay for the detour.</p>
            <p>One honest catch while we&rsquo;re here: the net only catches the kind of trouble you <em>named</em>. Feed this function text instead of numbers and a different error (<code>TypeError</code>) sails right past <code>except ZeroDivisionError</code> &mdash; and the program still crashes. The fence is real, but it&rsquo;s not a dome.</p>
          </>
        ),
        rigorous: (
          <>
            <p>Exact cost: the non-raising path does no handler work at all &mdash; CPython (3.11+) compiles <code>try</code> to an exception table consulted only when something raises, so the happy path runs as if the <code>try</code> weren&rsquo;t there. The raising path pays: build the exception object, unwind to the handler, match, run it.</p>
            <p>Edge cases: a non-matching type (<code>TypeError</code> here) is not caught and still kills the run; only code <em>inside</em> the block is covered; a handler that itself raises starts a new propagation; a bare <code>except:</code> matches everything &mdash; including the bugs you wanted loud.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>Now the bad day: with a <code>0</code> underneath, <code>10 / 0</code> fails mid-line. Instead of the whole program dying, control leaps to <code>except</code>, which hands back <code>0</code> &mdash; and the program <strong>shrugs and keeps going</strong>.</>,
          rigorous: <><code>10 / 0</code> raises <code>ZeroDivisionError</code>; control lands on the matching handler, which returns <code>0</code> &mdash; the caller sees a value, not a crash. One construct, both runs covered.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The seed of idea 6 of 7 &mdash; amortization:</strong> keep the common path cheap and let the rare failure pay &mdash; averaged over many runs, the protection is nearly free.
            </div>
          </>
        ),
        intuitive: (
          <>
            <p>The move you just derived has a proper name: <strong>exception handling</strong> &mdash; <code>try</code> the risky thing, let <code>except</code> catch the failure you anticipated, and the program stays standing in a messy world.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The seed of big idea 6 of 7 &mdash; <Term word="amortization">amortization</Term>:</strong> 999 clean runs pay nothing extra; the one bad call pays the detour &mdash; spread out, safety is nearly free.
            </div>
          </>
        ),
        rigorous: (
          <>
            <p><strong>The pattern: exception handling.</strong> Wrap the risky region, name the recoverable failure, return a defined fallback &mdash; callers get a total function: every input yields a value, none a crash. Recover only where you can act; elsewhere let the exception propagate to a caller that can.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 6 of 7, foreshadowed &mdash; amortization:</strong> zero cost on the non-raising path, bounded cost on the rare raise &mdash; the average tends to the happy path&rsquo;s.
            </div>
          </>
        ),
      }),
