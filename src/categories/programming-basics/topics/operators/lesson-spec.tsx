  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>Your named boxes can <em>hold</em> a score or a total, but a program also has to <strong>do</strong> things with them: add the score, check the total. An <Term word="operator">operator</Term> is the doing part: a symbol like <code>+</code> or <code>&gt;</code> (&ldquo;greater than&rdquo;) that takes one or two values and works out one new value. Three families of them do almost everything.</>,
        }),
      }],
      detail: reg({
        base: <><p>An <Term word="expression">expression</Term> is any piece of code that works out to a value: <code>3 + 4 * 2</code>, <code>total &gt; 10</code>. Operators are how you build them.</p></>,
        intuitive: (
          <>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>The familiar ones: <code>+ - * /</code>. They obey the same <strong>order</strong> you learned in school: <code>*</code> and <code>/</code> happen before <code>+</code> and <code>-</code>, so <code>3 + 4 * 2</code> is <code>11</code>. Use parentheses to force a different order.</>,
          intuitive: <>The maths family: <code>+ - * /</code> (<code>*</code> is multiply, <code>/</code> is divide). They run in the order you learned at school: multiply and divide first, then add and subtract, never simply left to right. Commit to a guess below, then watch the row resolve. Parentheses, as in <code>(3 + 4) * 2</code>, force a different order.</>,
      ...
      detail: reg({
        ...
        intuitive: (
          <>
            ...
            <p>Two extras you&rsquo;ll meet constantly: <code>7 // 2</code> is <code>3</code>, which divides then <em>drops</em> the leftover. <code>7 % 2</code> is <code>1</code>, which keeps <em>only</em> the leftover. Leftover-maths sounds niche but shows up everywhere: is a number even? did the counter wrap around?</p>
          </>
        ),
        rigorous: (
          <>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Comparison operators don&rsquo;t give a number. There are six of them: <code>&gt;</code>, <code>&lt;</code>, <code>&gt;=</code>, <code>&lt;=</code>, <code>==</code> (equal), <code>!=</code> (not equal). They give a <Term word="boolean">boolean</Term>: <code>True</code> or <code>False</code>.</>,
          intuitive: <>Six question-askers: <code>&gt;</code> greater, <code>&lt;</code> less, <code>&gt;=</code> at least, <code>&lt;=</code> at most, <code>==</code> equal, <code>!=</code> not equal. The answer is never a number. It&rsquo;s a <Term word="boolean">boolean</Term> (a yes/no value): <code>True</code> or <code>False</code>. And it&rsquo;s a real value: <code>is_big = total &gt; 10</code> stores the answer in a box.</>,
        }),
      }],
      detail: reg({
        ...
        intuitive: (
          <>
            <p>One trap to dodge early: <code>=</code> and <code>==</code> look like twins but do opposite jobs. One equals sign <strong>stores</strong>: <code>total = 10</code> puts 10 in the box. Two equals signs <strong>ask</strong>: <code>total == 10</code> checks, and answers <code>True</code> or <code>False</code>. Read <code>=</code> as &ldquo;store&rdquo; and <code>==</code> as &ldquo;is it equal?&rdquo; and the classic beginner bug never bites.</p>
            <p>Asking is gentle, too: checking <code>total &gt; 10</code> doesn&rsquo;t change <code>total</code>, just like reading a box never emptied it.</p>
          </>
        ),
      }),
      codeLabels: ["compare"],
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          rigorous: <>Comparisons (<code>&gt; &lt; &gt;= &lt;= == !=</code>) produce booleans; <code>and</code>/<code>or</code>/<code>not</code> compose them. Evaluation short-circuits: <code>and</code> stops at a false left operand, <code>or</code> at a true one, so the right side never runs. Comparisons also chain: <code>0 &lt; x &lt; 10</code> means <code>0 &lt; x and x &lt; 10</code>.</>,
        }),
      }],
      detail: reg({
        ...
        intuitive: (
          <>
            <p>This is where conditions start to read like English: <code>age &gt;= 18 and has_ticket</code> &mdash; &ldquo;an adult, with a ticket.&rdquo; However long the line grows, it still works out to one single <code>True</code> or <code>False</code>: one answer to one (compound) question.</p>
          </>
        ),
        rigorous: (
          <>
            <p>Edge cases that bite: <code>=</code> binds, <code>==</code> compares. Python rejects <code>=</code> inside an expression, so the classic C slip is a syntax error here, not a silent bug. Short-circuiting is load-bearing, not an optimisation: <code>x != 0 and 1 / x &lt; 1</code> is safe precisely because the division never runs when <code>x</code> is 0.</p>
            <p>Strictly, <code>and</code>/<code>or</code> return an <em>operand</em>, not necessarily a bool (<code>0 or &quot;default&quot;</code> yields <code>&quot;default&quot;</code>), and every value is truthy or falsy (<code>0</code>, <code>&quot;&quot;</code>, empty containers are falsy). That pair of rules is what makes the default-value idiom work.</p>
          </>
        ),
      }),
      codeLabels: ["logical"],
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Arithmetic gives you new <strong>numbers</strong>; comparison and logical give you <strong>booleans</strong>. Those booleans are exactly the fuel an <code>if</code> burns, which is where decisions come from.</>,
          intuitive: <>Every operator you met hands back one of two kinds of value: <strong>numbers</strong> (the maths family) or <strong>booleans</strong> (comparisons and the glue words). The booleans are exactly what the next lesson feeds to <code>if</code>: the line where a program starts choosing.</>,
          rigorous: <>Two result kinds: numbers (arithmetic) and booleans (comparison, logical). Expressions of any size stay tractable because evaluation is compositional: each operator consumes the values of its sub-expressions. The booleans feed <code>if</code> next.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>You now have values (variables, types) and ways to combine them (operators). Put a boolean operator inside an <code>if</code> and the program starts making choices: that&rsquo;s conditionals.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The seed of idea 4 of 7, decomposition:</strong> a big expression is never solved in one go. Python works out the small pieces and combines their results.
            </div>
          </>
        ),
        intuitive: (
          <>
            <p>Put together, you now own two of the three ingredients of every program: values in named boxes, and operators to combine what they hold. Each full combination (<code>3 + 4 * 2</code>, <code>is_big and total &lt; 100</code>) is an <Term word="expression">expression</Term>: code that works out to one value. Next lesson, a True/False goes inside an <code>if</code>, and the program takes one road and skips the other.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The first glimpse of idea 4 of 7, break it into smaller pieces:</strong> even <code>3 + 4 * 2</code> isn&rsquo;t solved in one go. Python solves a small piece (<code>4 * 2</code>), then a simpler leftover (<code>3 + 8</code>). Big problems as little ones, combined &mdash; you&rsquo;ll meet this move again and again.
            </div>
          </>
        ),
        rigorous: (
          <>
            <p>The model, complete: variables name values; operators combine them into expressions; expressions evaluate inside-out, every operand resolved before its operator applies, and conditionals consume the booleans. Result types partition the families; precedence and short-circuiting decide order.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 4 of 7, foreshadowed, decomposition:</strong> an expression is evaluated as smaller expressions combined; later, whole problems get the same treatment.
            </div>
          </>
        ),
      }),
