  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: <>A program that can&rsquo;t remember anything can&rsquo;t do much. You need a place to <strong>keep a value</strong>, like a player&rsquo;s score, a name, or a total, and get it back later.</>,
      }],
      detail: (
        <>
          <p>Think of a labelled box. You write a name on it, say <code>score</code>, and you can drop a value inside, look at what&rsquo;s in there whenever you want, and swap it for something new.</p>
    ...
    },
    {
      ...
      detail: reg({
        ...
        intuitive: (
          <>
            <p>One warning about <code>=</code>: in maths it claims two sides are equal. Here it&rsquo;s a command, an <Term word="assignment">assignment</Term>: &ldquo;work out whatever is on the right, then store it under the name on the left.&rdquo;</p>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          rigorous: <>The right-hand side is evaluated <em>first</em>, against the current binding (<code>score</code> &rarr; <code>0</code>); then <code>=</code> rebinds the name to the result. One name, one binding: assignment replaces it.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            ...
            <p>The previous value is simply overwritten. A box only ever holds one thing at a time.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Reading <code>score = score + 10</code> like maths feels broken: nothing can equal itself plus ten. But it&rsquo;s not a claim, it&rsquo;s an instruction with three small steps: <strong>read</strong> the box (it holds 0), <strong>work out</strong> 0 + 10, <strong>store</strong> the 10 back in.</p>
            <p>And the 0? A box has no layers and no memory of its own. One value lives there at a time, so storing the 10 pushes the 0 out for good. If you wanted to keep the 0, you&rsquo;d need to put it in another box first.</p>
          </>
        ),
        rigorous: (
          <>
            <p><code>=</code> is binding, not equality: evaluate the right-hand side fully, then bind the name on the left to the result. Here <code>score + 10</code> reads the current binding (0), yields 10, and <code>score</code> is rebound to it. The old value isn&rsquo;t altered; it&rsquo;s simply no longer reachable through this name.</p>
            <p><strong>Rebinding vs mutation:</strong> rebinding points the name at a new value; mutation changes a value in place while the binding stays put. Python ints are immutable, so <code>score = score + 10</code> is always a rebind. The distinction starts to matter with mutable containers later.</p>
          </>
        ),
      }),
      codeLabels: ["update"],
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Using a variable just means writing its name. <code>score</code> stands in for its value (<code>10</code>), so <code>score * 2</code> is <code>20</code>, stored in a brand-new box, <code>bonus</code>.</>,
          intuitive: <>Anywhere you write a box&rsquo;s name, the program swaps in whatever the box holds at that moment. <code>score</code> holds <code>10</code>, so <code>score * 2</code> works out to <code>20</code>, and that lands in a brand-new box, <code>bonus</code>.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>Wherever you write <code>score</code>, the program swaps in whatever value the box currently holds. Reading it doesn&rsquo;t empty the box: <code>score</code> is still <code>10</code> afterwards.</p>
          </>
        ),
        intuitive: (
          <>
            <p>This is the payoff of naming things: store a value once, then <em>reuse it by name</em> wherever you need it, with no re-typing it and no working it out again.</p>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>Every line so far made a box or changed one. Those boxes are everything the program knows right now, and <code>print</code> simply reads three of them back out by name.</>,
          rigorous: <>Five statements, three live bindings. <code>print</code> resolves each name to its current value at call time. Names carry no type, the values do, so any name could be rebound to a different kind of value next line.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            ...
            <p>Tip: when a value should <em>never</em> change, programmers write its name in <code>UPPER_CASE</code> (like <code>MAX_SCORE = 100</code>) as a signal. That&rsquo;s a <Term word="constant">constant</Term>.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The seed of idea 1 of 7, information reuse:</strong> name a value once and nothing has to be figured out twice.
            </div>
          </>
        ),
        intuitive: (
          <>
            ...
            <p>One habit worth copying early: when a value should <em>never</em> change, programmers write its name in capitals, like <code>MAX_SCORE = 100</code>, as a signal to each other. That&rsquo;s called a <Term word="constant">constant</Term>.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The first big idea (1 of 7), reuse what you already know:</strong> once a value has a name, you never work it out again. You just ask for it.
            </div>
          </>
        ),
        rigorous: (
          <>
            <p><strong>The model, complete:</strong> a variable is a name bound to a value. Assignment binds (or rebinds) the name to the fully evaluated right-hand side; reading a name yields its current value and never changes the binding; names are untyped, and the values carry the type.</p>
            <p><code>UPPER_CASE</code> names mark constants: a convention only, nothing enforced.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 1 of 7, foreshadowed, information reuse:</strong> bind once, reference everywhere; nothing is computed twice.
            </div>
          </>
        ),
      }),
