  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Someone hands you a sentence and asks: &ldquo;Is the word <code>brown</code> in here &mdash; and where?&rdquo; You don&rsquo;t see five neat boxes. You see one long stream of letters. So how do you actually go find <code>brown</code> inside it?</>,
          intuitive: <>Here&rsquo;s the sentence, letter by letter, and one question: is the word <code>brown</code> in there &mdash; and at which spot? Notice what you&rsquo;re given. Not five neat boxes spelling the word out &mdash; just one long stream of letters. How would you hunt it down by hand?</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            <p>Start with what&rsquo;s actually in front of you: one long row of letters &mdash; <code>the quick brown fox</code> &mdash; and one question: &ldquo;Is <code>brown</code> in here? And if it is, <em>where</em>?&rdquo;</p>
            <p>The word isn&rsquo;t handed to you in its own five boxes. It&rsquo;s buried somewhere in the stream, and nothing marks where it starts. That&rsquo;s the puzzle &mdash; and chasing it is going to show us what text really <em>is</em> underneath.</p>
          </>
        ),
      }),
      codeLabels: ["source"],
    ...
    },
    {
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Drag the slider to pick a starting box. The five letters under the highlight are your candidate. Slide it across. Landing on box 12 is no harder than box 2 &mdash; jumping straight to any single letter is one quick, instant lookup.</>,
            intuitive: <>Drag the slider under the row. The five highlighted letters are your candidate &mdash; the stretch you&rsquo;d compare to <code>brown</code>. Now feel the cost: landing on box 12 takes exactly as long as box 2. Any letter, one instant look &mdash; no counting past the others.</>,
          }),
        },
        {
      ...
      detail: reg({
        ...
        intuitive: (
          <>
            <p>Drag the slider and watch the highlight jump. Wherever it lands, the five letters underneath are your <strong>candidate</strong> &mdash; the stretch you&rsquo;d line up against <code>brown</code> at that spot.</p>
            <p>The thing to feel is the cost. Hopping to box 12 is no slower than hopping to box 2 &mdash; you never wade through the letters in between. &ldquo;Give me letter 12&rdquo; goes straight to it, one quick step &mdash; exactly like grabbing a numbered slot on last lesson&rsquo;s shelf.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The instinct question:</strong> the letters didn&rsquo;t move and the reading never slowed down &mdash; so is a string anything <em>more</em> than a plain row of letter-boxes? Anything at all?
            </div>
          </>
        ),
      }),
    ...
    },
    {
      ...
      takeaway: reg({
        ...
        rigorous: "A string: char array + enforced immutability — every edit allocates a copy.",
      }),
      ...
      panels: [{
        ...
        body: reg({
          base: <>That&rsquo;s the secret: boxed letters in order, side by side &mdash; so every array move still works. The one twist: a string is <strong>immutable</strong> (can&rsquo;t be changed once made). <code>s[0]=&apos;H&apos;</code> simply isn&rsquo;t allowed; to &ldquo;change&rdquo; it you build a brand-new string.</>,
          intuitive: <>That&rsquo;s the secret: letters boxed up in order, side by side &mdash; every array move you know still works on text. The one twist: a string is <strong>immutable</strong> &mdash; once it&rsquo;s made, it can never be edited. <code>s[0] = &apos;H&apos;</code> is refused outright; to &ldquo;change&rdquo; a string you build a brand-new one with the change baked in.</>,
          rigorous: <>A <code>str</code> is a contiguous character array &mdash; index, length, slice behave exactly as on a list. The addition is an enforced invariant: <strong>immutability</strong>. No write ever lands in place (<code>s[0] = &apos;H&apos;</code> raises <code>TypeError</code>); every &ldquo;edit&rdquo; allocates a new string and copies into it.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            <p>The answer to the instinct question is &ldquo;almost nothing.&rdquo; A string is letters boxed up in order, side by side &mdash; so every move you learned for arrays works here unchanged: jump to letter <code>i</code>, ask the length, grab a stretch.</p>
            <p>The one twist is the lock you just predicted. In many languages (Python, Java, JavaScript) a string is <strong>immutable</strong> &mdash; once it&rsquo;s made, it can never be edited. <code>s[0] = &apos;H&apos;</code> is flat-out refused. Want &ldquo;Hello&rdquo; instead of &ldquo;hello&rdquo;? You don&rsquo;t fix the old string &mdash; you build a brand-new one with the change baked in.</p>
            <p>Keep the word for a promise like this: an <Term word="invariant">invariant</Term> &mdash; a fact guaranteed to stay true, no matter what. &ldquo;This string never changes&rdquo; is exactly one, and the next beat counts what the guarantee costs.</p>
          </>
        ),
        rigorous: (
          <>
            <p><strong>The model.</strong> A string is a contiguous array of characters &mdash; the array contract carries over whole: position is computed, not searched, so indexing, length, and slicing behave exactly as they do on a list. Nothing about text changes the machinery.</p>
            <p><strong>The invariant.</strong> Contents are fixed at construction. No write ever lands in place &mdash; <code>s[0] = &apos;H&apos;</code> raises <code>TypeError</code> &mdash; so every transformation allocates a new string and copies into it. What the constraint buys: strings hash and share safely (a value that cannot change can key a dict, sit in a set, or be passed anywhere without a defensive copy), and every cost is explicit &mdash; if it transforms, it copies.</p>
          </>
        ),
      }),
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Reading one letter <code>s[i]</code> is <Term word="O(1)"><strong>O(1)</strong></Term> &mdash; &ldquo;O(...)&rdquo; just describes how cost grows; O(1) means instant, the same however long the string is. But every edit copies the whole thing into a new string. Watch the counter climb.</>,
            intuitive: <>Reading one letter is one step however long the string is &mdash; that flat cost is written <Term word="O(1)"><code>O(1)</code></Term>. The counter under the row is the real story: every &ldquo;edit&rdquo; quietly copies <em>every letter</em> into a fresh string. Watch how fast the copies pile up.</>,
            rigorous: <><code>s[i]</code> and <code>len(s)</code> are one address computation each &mdash; <Term word="O(1)"><code>O(1)</code></Term>. Everything else allocates: <code>s[i:j]</code> copies j &minus; i characters &mdash; <Term word="O(j − i)"><code>O(j &minus; i)</code></Term>; <code>a + b</code> copies both &mdash; <Term word="O(n + m)"><code>O(n + m)</code></Term>. The counter tallies copies; watch <code>+=</code> pay the full length every round.</>,
          }),
        },
        {
          ...
          body: reg({
            ...
            rigorous: <>Repeated <code>+=</code> to length n: 1 + 2 + &hellip; + n copies = n(n+1)/2 &mdash; <Term word="O(n²)"><code>O(n&sup2;)</code></Term>. Accumulate in a list, <code>&quot;&quot;.join</code> once &mdash; O(n) total.</>,
          }),
        },
      ],
      detail: reg({
        ...
        intuitive: (
          <>
            <p><strong>Reading one letter</strong> is one step, whether the string holds 5 letters or 5 million. Cost that stays flat like that is written <Term word="O(1)"><code>O(1)</code></Term> &mdash; tap the chip; it just means &ldquo;same work at any size.&rdquo;</p>
            <p><strong>Now the counter.</strong> Swapping one letter of &ldquo;hello&rdquo; copied 5 characters; sticking a &ldquo;!&rdquo; on copied 6; the next one copied 7. Build a string letter by letter with <code>+=</code> and the copies climb 1, 2, 3 &hellip; up to n &mdash; the total grows like the <em>square</em> of the length, written <Term word="O(n²)"><code>O(n&sup2;)</code></Term>. A 10,000-letter string built that way costs about fifty million copies.</p>
            <p>Two quieter prices while we&rsquo;re counting: grabbing a stretch <code>s[i:j]</code> copies just that stretch &mdash; <Term word="O(j − i)"><code>O(j &minus; i)</code></Term> &mdash; and gluing <code>a + b</code> copies both halves &mdash; <Term word="O(n + m)"><code>O(n + m)</code></Term>. And two edges that bite: the empty string has no letter 0 at all (asking for it is an error), and hunting a word that isn&rsquo;t there still walks the whole text before saying no.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The fix:</strong> never grow a string one letter at a time. Pile the pieces into a list as you go, then glue them all in one move with <code>&quot;&quot;.join(...)</code> &mdash; one build instead of thousands.
            </div>
          </>
        ),
        rigorous: (
          <>
            <p><strong>Exact costs.</strong> <code>s[i]</code>, <code>len(s)</code>: O(1) address arithmetic, as for any array. <code>s[i:j]</code>: a new string, j &minus; i characters copied &mdash; O(j &minus; i). <code>a + b</code>: allocate n + m, copy both &mdash; O(n + m). Building length n by repeated <code>+=</code>: 1 + 2 + &hellip; + n copies = n(n+1)/2 &mdash; O(n&sup2;); collecting parts and joining once is O(n) total. (CPython sometimes patches <code>+=</code> in place when the reference count allows; never design against it.) Substring search: up to m letter-checks at each of n &minus; m + 1 starts &mdash; O(n&middot;m) naive; the library&rsquo;s search is tuned, but still grows with the text.</p>
            <p><strong>Edges.</strong> The empty string: <code>len</code> 0; <code>s[0]</code> raises <code>IndexError</code>; <code>find</code> returns &minus;1, not an exception &mdash; two different not-found behaviours in one API. Slicing never raises: out-of-range bounds clamp (<code>s[5:999]</code> is fine), which silently absorbs the off-by-one bugs that indexing would crash on. Negative indices alias from the end. Searching for an absent word always pays the full scan.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>The mental model: an array of characters, immutable from the outside, indexed instantly. Almost every array cost rule carries straight over &mdash; the only new rule is that &ldquo;changing&rdquo; one letter means quietly building a whole new string.</>,
          intuitive: <>The name: a <strong>string</strong>. Keep the model: a row of letter boxes &mdash; instantly indexable, never editable. That locked-box promise is a guarantee that never breaks &mdash; an <Term word="invariant">invariant</Term> &mdash; and it&rsquo;s your first taste of idea 3 of 7.</>,
          rigorous: <>A string: an immutable character array &mdash; O(1) index and length, a copy for every transform, list + join for accumulation. The immutability is an enforced <Term word="invariant">invariant</Term> &mdash; the seed of idea 3 of 7, <strong>monotonicity &amp; invariants</strong>.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            <p>The name is the obvious one: a <strong>string</strong>. The model to keep is one line &mdash; <em>a row of letter boxes, instantly indexable, never editable.</em> Everything you banked about arrays carries straight over; the only new rule is that &ldquo;changing&rdquo; a letter quietly rebuilds the whole string &mdash; which is why the cost table above holds no surprises now.</p>
            <p>And the twist is bigger than text. &ldquo;This value never changes&rdquo; is a promise the language <em>enforces</em> &mdash; an <Term word="invariant">invariant</Term>, a fact you can lean on without ever re-checking it. You&rsquo;ve seen what the kept promise costs (copies) and what it buys (a value that can&rsquo;t change behind your back).</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The seed of idea 3 of 7 &mdash; keep a guarantee:</strong> immutability is your first <em>enforced</em> invariant. Later lessons keep promises like this on purpose &mdash; a guarantee you can trust is work you can skip.
            </div>
        ...
        rigorous: (
          <>
            <p>String: an immutable, contiguous character array. O(1) index and length; every transform &mdash; slice, concat, <code>upper</code> &mdash; allocates and copies; accumulation goes through a list and one join. Immutability also makes strings hashable: dict keys, set members, sharing without defensive copies.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 3 of 7, foreshadowed &mdash; monotonicity &amp; invariants:</strong> immutability is an invariant the language maintains for you. The algorithm lessons maintain their own &mdash; a guarantee held at every step is work skipped at every step.
            </div>
          </>
        ),
      }),
