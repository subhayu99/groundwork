  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>You&rsquo;re shelving a thousand books and a friend says &ldquo;hand me book number 487.&rdquo; You don&rsquo;t care what it is &mdash; only that it sits at that <strong>position</strong>. How fast can you reach it? (Picture this short row 1,000 long.)</>,
          intuitive: <>A friend points at a shelf of a thousand books: &ldquo;hand me book number 487.&rdquo; Which book it is doesn&rsquo;t matter &mdash; only the spot it sits in. The whole lesson is one question: how fast can you put your hand on spot 487? (Picture this short row stretched 1,000 long.)</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            ...
            <p>Notice what the question is <em>not</em> about. Not the title, not the cover, not what&rsquo;s inside. Only the spot &mdash; the 487th place in the row. Every storage lesson starts at this question: once you know <em>where</em> something lives, how fast can you put your hand on it?</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      connector: reg({
        ...
        structured: "Pile-like storage makes you count from the top — reaching position i costs i steps. Change one thing: give every position a fixed home.",
      }),
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Now lay the books on a long shelf, every slot the same size: slot 0, slot 1 &hellip; slot 999. The slot&rsquo;s number is its <strong>index</strong> (the position, counting from 0). <em>Click any slot</em> &mdash; you walk straight there in one step, no counting.</>,
            intuitive: <>Lay the books on a long shelf instead, every slot exactly the same width: slot 0, slot 1 &hellip; slot 999. A slot&rsquo;s number is its <strong>index</strong> &mdash; just &ldquo;which spot,&rdquo; counting from 0. <em>Click any slot</em>: you land on it in one step. No lifting, no counting through the others.</>,
          }),
        },
        {
      ...
      detail: reg({
        ...
        intuitive: (
          <>
            <p>Same books, new arrangement: a long shelf where every slot is exactly the same width &mdash; slot 0, slot 1, slot 2&hellip; slot 999. A slot&rsquo;s number is its <strong>index</strong>, and that&rsquo;s all an index is: &ldquo;which spot,&rdquo; counting from zero.</p>
            <p>Try it &mdash; click any slot in the picture. You never pass through the slots before it; you just arrive. Book 487 no longer costs 487 lifts. It costs one step &mdash; and so does every other book on the shelf.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The instinct question:</strong> the books didn&rsquo;t change &mdash; only their <em>arrangement</em> did. Why would that alone make finding one instant?
            </div>
          </>
        ),
      }),
    ...
    },
    {
      ...
      takeaway: reg({
        ...
        rigorous: "address(i) = base + i × size — O(1) random access; the layout is the lookup.",
      }),
      connector: reg({
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>That row of same-size slots is an <strong>array</strong>. The slots sit side by side with no gaps, so the computer never walks the row &mdash; it <em>calculates</em> where slot <code>i</code> is: start of the row + i &times; one slot&rsquo;s width. Ten books or a million &mdash; the same one step. Work that stays flat like that is called <Term word="O(1)"><code>O(1)</code></Term>, &ldquo;constant time.&rdquo;</>,
          rigorous: <>Contiguous memory, fixed-size cells: slot <code>i</code>&rsquo;s address is computed, not searched &mdash; <code>base + i × size</code>, one multiply, one add, one load, identical for n = 10 or n = 10⁶. That flat count is <Term word="O(1)"><code>O(1)</code></Term> random access.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            <p>That row of equal slots is an <strong>array</strong>. Two promises make it work: every slot is the same width, and the slots sit right next to each other with no gaps &mdash; in <strong>memory</strong>, the computer&rsquo;s own long row of numbered storage spots.</p>
            <p>Those two promises turn &ldquo;walk there&rdquo; into &ldquo;calculate it.&rdquo; Where is slot 487? The start of the row, plus 487 slot-widths: <code>base + 487 &times; size</code>. The computer does that one multiplication and lands on the spot &mdash; it never touches the 486 slots before it, just like you never lifted those books.</p>
            <p>Ten books or a million, that math is the same single step. Work that stays flat as the data grows is called <strong>constant time</strong>, written <Term word="O(1)"><code>O(1)</code></Term> &mdash; tap the chip; it just means &ldquo;one step, every time.&rdquo;</p>
          </>
        ),
        rigorous: (
          <>
            <p><strong>The layout invariant.</strong> Element <code>k</code> occupies bytes <code>[base + k·size, base + (k+1)·size)</code> &mdash; fixed element size, no gaps, stable base. Every operation preserves it, so a position is never searched for: it is computed. <code>arr[i]</code> compiles to exactly that address expression.</p>
            <p>The invariant is also the constraint. Equal-size cells are what make the stride arithmetic valid (Python keeps it by storing fixed-size <Term word="pointer">references</Term> in the cells, never the objects themselves), and contiguity is what the CPU rewards: neighbouring indices share cache lines, so sequential scans run near memory bandwidth &mdash; <Term word="memory locality">locality</Term>.</p>
          </>
        ),
      }),
      arrows: [{ x1: G.cx(TARGET), y1: 150, x2: G.cx(TARGET), y2: G.y - 4 }],
    ...
    },
    {
      ...
      takeaway: reg({
        ...
        rigorous: "Index ops are O(1); middle insert/delete shifts the tail — O(n); append amortized O(1).",
      }),
      connector: reg({
        base: "Reaching a slot is free — but not every operation gets to ride that arithmetic, so let's price them out.",
        rigorous: "The address math prices reads. Mutation moves memory — predict the cost before the playback.",
      }),
      actionLabel: reg({
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Read or write by index is one step at any size &mdash; written <strong>O(1)</strong> (&ldquo;cost stays the same&rdquo;). Inserting in the middle differs: call the item count <code>n</code>; every later item must shift over to make room, so cost grows in step with <code>n</code> &mdash; written <Term word="O(n)"><strong>O(n)</strong></Term>. Watch the tail shift.</>,
            intuitive: <>Grabbing any slot by its number is one step at any size &mdash; <Term word="O(1)"><code>O(1)</code></Term>. Squeezing a book into the <em>middle</em> is different: every later book must step right to make room. Call the book count <code>n</code> &mdash; the work grows with it, written <Term word="O(n)"><code>O(n)</code></Term>. Predict it, then watch.</>,
            rigorous: <>Read or write by index is the same address computation &mdash; <Term word="O(1)"><code>O(1)</code></Term>, already counted. Insert at index <code>i</code>: cells <code>i..n−1</code> each step right &mdash; n &minus; i writes, n at the front; delete mirrors it. The playback counts the shifts; that linear count is what <Term word="O(n)"><code>O(n)</code></Term> names.</>,
          }),
        },
        {
          left: 540, top: 372, width: 290, variant: "note",
          body: reg({
            base: <>Append at the end is O(1) too &mdash; on average. Once in a while the shelf is full and the books are copied to a bigger one.</>,
            intuitive: <>Adding at the <strong>end</strong> barely disturbs the row &mdash; usually one step. Rarely the shelf is full and everything is copied to a bigger one; spread out, that cost stays tiny (<Term word="amortized">amortized</Term>).</>,
            rigorous: <>Append: <Term word="amortized">amortized</Term> O(1) &mdash; capacity doubles on overflow, so n appends cost O(n) total, though any single append may pay a full copy.</>,
          }),
        },
      ],
      detail: reg({
        ...
        intuitive: (
          <>
            ...
            <ul>
              <li><strong>Grab / replace by slot number:</strong> one step at any size &mdash; <Term word="O(1)"><code>O(1)</code></Term> &mdash; it&rsquo;s just the start + slot &times; width calculation.</li>
              <li><strong>Add at the end:</strong> usually one step too &mdash; nothing else moves. (Rarely the shelf fills up and every book is copied to a bigger one; spread across many adds, the average stays tiny &mdash; that&rsquo;s <Term word="amortized">amortized</Term> cost.)</li>
              <li><strong>Squeeze one into the middle:</strong> every book after the spot steps right &mdash; you just watched it. For a thousand books that can be hundreds of moves: <Term word="O(n)"><code>O(n)</code></Term>.</li>
              <li><strong>Find a value with no slot number:</strong> nothing to calculate, so it&rsquo;s look, look, look &mdash; also <Term word="O(n)"><code>O(n)</code></Term>.</li>
            </ul>
            <p>One pattern under all four: a known <em>position</em> is calculated &mdash; instant. <em>Moving</em> books or <em>hunting</em> for one costs steps that grow with the row.</p>
          </>
        ),
        rigorous: (
          <>
            <p><strong>Exact costs.</strong> Read/write by index: O(1). Append: <Term word="amortized">amortized</Term> O(1) &mdash; geometric growth makes n appends O(n) total, though any single append may pay a full copy. Insert/delete at index <code>i</code>: the tail <code>i..n−1</code> shifts one slot &mdash; n &minus; i moves, so O(n) worst case at the front and O(1) at the back. Membership with no index: O(n) scan &mdash; the layout buys nothing there.</p>
            <p><strong>Edges.</strong> The empty array: every index is invalid and a scan trivially misses. Out-of-range reads raise <code>IndexError</code> in Python &mdash; no clamping, no silent growth. Negative indices alias from the end (<code>arr[-1]</code> is the last cell), which masks off-by-one bugs instead of crashing them. A search for an absent value always costs the full n.</p>
          </>
        ),
      }),
      ...
      detail: reg({
        ...
        intuitive: (
          <>
            ...
            <p>Reach for something else when you&rsquo;re constantly squeezing things into the middle or pulling them out of it &mdash; every single edit pays the tail-shuffle, <Term word="O(n)"><code>O(n)</code></Term>, again and again.</p>
            <p>Middle-heavy work is exactly what other shapes are built for: <strong>linked lists</strong> (each item points to the next, so splicing one in is cheap), <strong>trees</strong> (items arranged by branching), and <Term word="hash map"><strong>hash maps</strong></Term>. They&rsquo;re all later lessons &mdash; for now, know the array&rsquo;s sweet spot and its weak spot.</p>
          </>
        ),
      }),
      codeLabels: ["loop"],
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>The name: an <strong>array</strong>. Python&rsquo;s everyday <code>list</code> is a <Term word="dynamic array"><strong>dynamic array</strong></Term> &mdash; the same shelf, it just grows itself when you append. Every <Term word="arr[i]"><code>arr[i]</code></Term> in code is that start + i &times; width jump &mdash; and that bargain (a strict layout for instant access) is the seed of idea 5 of 7: <strong>trade space for time</strong>.</>,
          rigorous: <>An array &mdash; Python&rsquo;s <code>list</code> is a <Term word="dynamic array">dynamic array</Term>: same layout plus geometric growth, same cost model. Every <Term word="arr[i]"><code>arr[i]</code></Term> is the <code>base + i × size</code> computation: space spent on a rigid layout, time bought back &mdash; the seed of <strong>trade-space-for-time</strong>, idea 5 of 7.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            ...
            <p>And file the deal it rests on: the array <em>spends</em> a strict, gap-free layout and <em>buys</em> instant access to any position. That trade is the seed of <strong>idea 5 of 7, trade space for time</strong> &mdash; a few lessons from now, hash maps grow the same seed into something that feels like magic.</p>
            <p>Open the Code panel to see the handful of operations you&rsquo;ll actually reach for day to day.</p>
          </>
        ),
        rigorous: (
          <>
            <p>Array: contiguous, fixed-stride storage &mdash; access is address arithmetic. Python&rsquo;s <code>list</code> stores fixed-size references contiguously and over-allocates geometrically; C arrays fix n up front; C++ <code>vector</code> and Java <code>ArrayList</code> are the same dynamic-array deal. One cost model covers them all.</p>
            <p>The principle this seeds &mdash; <strong>trade space for time, idea 5 of 7</strong>: commit memory to a strict layout so position is computed, never searched. Arrays are the first installment; hash maps pay more space still to make even membership O(1).</p>
          </>
        ),
      }),
      arrows: [{ x1: G.cx(TARGET), y1: 150, x2: G.cx(TARGET), y2: G.y - 4 }],
