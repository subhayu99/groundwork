  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>A phone book: ten thousand names, in no order. Someone asks for Alice&rsquo;s number. You don&rsquo;t know her page, or even if she&rsquo;s listed. Every answer costs work. How much does finding one name cost?</>,
          intuitive: <>A phone book with ten thousand names, and nobody sorted it. Someone asks for Alice&rsquo;s number. You don&rsquo;t know her page. You don&rsquo;t even know if she&rsquo;s listed. The whole lesson is one question: how much work does finding one name cost?</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>Picture a real phone book with ten thousand names and numbers, and, to make it hard, they&rsquo;re in <em>no</em> order at all. Someone walks up and asks, &ldquo;What&rsquo;s Alice&rsquo;s number?&rdquo;</p>
            <p>You don&rsquo;t know which page she&rsquo;s on. You don&rsquo;t even know if she&rsquo;s in the book at all. The only way to answer is to do some <strong>work</strong>: flip pages, read names, compare. The whole question of this lesson is: how much work does finding <em>one</em> name have to cost?</p>
          </>
        ),
        intuitive: (
          <>
            <p>Hold the picture in your head: a real phone book, ten thousand names with a number next to each, and, to make it hard, in no order at all. Someone walks up: &ldquo;What&rsquo;s Alice&rsquo;s number?&rdquo;</p>
            <p>You can&rsquo;t guess her page. You can&rsquo;t even be sure she&rsquo;s in there. Any honest answer costs <strong>work</strong>: flipping pages, reading names, comparing. So before any clever trick, pin the question down: how much work should finding <em>one</em> name cost?</p>
          </>
        ),
      }),
      codeLabels: [],
    },
    {
      ...
      panels: [{
        ...
        body: <>The simple way: <strong>scan</strong>. Read names top to bottom until you hit Alice. If she&rsquo;s the 4,872nd, that&rsquo;s 4,872 reads; if missing, all ten thousand. The work grows in step with the count of names. We call that <Term word="O(n)"><strong>O(n)</strong></Term>. What if the name told you the page?</>,
      }],
      detail: (
        <>
          ...
          <p>Computer people write that worst case as <strong><code>O(n)</code></strong> (&ldquo;order n&rdquo;): the cost grows in step with <code>n</code>, the number of names. Double the book and you double the work. Sorting the book first would let you use a smarter search, about 14 checks for ten thousand, but that still isn&rsquo;t free, and you&rsquo;d have to keep it sorted every time someone is added.</p>
    ...
    },
    {
      ...
      connector: reg({
        ...
        structured: "A scan pays a read per name: 4,872 reads if Alice sits that deep, all ten thousand on a miss, so O(n). Change the move. Compute where she lives.",
      }),
      actionLabel: "Compute the address",
      takeaway: reg({
        base: "A hash turns the key itself into a box number: one hop, no searching.",
        intuitive: "A name's own letters become its box number, so you jump straight there, no searching.",
      }),
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Click a name and a <strong>hash function</strong>, a recipe that turns the letters into a number, hands you a box. It searches nothing; it computes a number, then takes it mod 16 (the remainder, landing in box 0&ndash;15). One hop, however huge the book.</>,
            intuitive: <>Click a name. A <strong>hash function</strong> is a fixed recipe that turns letters into a number, and it hands you a box on the spot. It looks nothing up; it computes, the way <code>3 + 4</code> always makes <code>7</code>. The raw number is huge, so we keep just the remainder after dividing by 16, which we call &ldquo;mod 16,&rdquo; and that always lands in box 0&ndash;15. One hop, however huge the book.</>,
          }),
        },
        {
      ...
      detail: reg({
        base: (
          <>
            <p>Click a name above. A <strong>hash function</strong> is a fixed recipe that turns the letters of a name into a number. It reads its characters and hands you a box number. It doesn&rsquo;t look anything up; it just <em>computes</em>, the way <code>3 + 4</code> always gives <code>7</code>. Same name in, same number out, every time.</p>
            <p>The raw number can be huge, so we take it <code>mod 16</code>, the <em>remainder</em> after dividing by 16, which always lands between 0 and 15, one of our boxes. Try a name that isn&rsquo;t even in the book: you still get a box instantly, because the recipe never searches. It computes.</p>
        ...
        intuitive: (
          <>
            <p>Try every chip. The same name always lands in the same box. That&rsquo;s the whole promise of a <strong>hash function</strong>: a fixed recipe, letters in, number out, no memory and no luck involved.</p>
            <p>The recipe&rsquo;s raw number can be enormous, so we keep only the <em>remainder</em> after dividing by 16, written &ldquo;mod 16.&rdquo; A remainder is always 0 to 15, so it always names one of our 16 boxes.</p>
            <p>Now try <code>zoe</code>, who isn&rsquo;t in the book at all. You <em>still</em> get a box instantly, because nothing was searched. The recipe doesn&rsquo;t know who&rsquo;s listed; it just turns letters into a number. Whether zoe is really there is answered by opening that one box.</p>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>A hash map is two parts glued together: a numbered row of boxes called <strong>buckets</strong> (the array you already know, where jumping to box <code>i</code> is one step), plus the letters-to-number recipe. Storing a name is hash, then drop. Predict below, then watch the names rain in.</>,
          rigorous: <>Two components: an array of <code>m</code> buckets (the index jump is address arithmetic, one step at any <code>m</code>) and a hash <code>h</code>. The invariant: a key lives in bucket <code>h(key) mod m</code>, or nowhere. Store: hash, jump, append. Find: the same hash and jump, then a walk of that one bucket only. Predict the wrinkle, then run it.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            ...
            <p>The first piece is an <strong>array of boxes</strong>, a numbered row of slots, here called <strong>buckets</strong>. An array is the most basic store there is, and its superpower is that jumping straight to box number <code>i</code> (written <code>arr[i]</code>) is <em>instant</em> &mdash; the computer leaps right to it without scanning past the others, no matter how many boxes there are.</p>
            <p>The second piece is the <strong>hash function</strong> from the last step: feed it a key, whether a name, a number, or anything else, and it produces a box number. A good hash spreads keys evenly so no single box gets overloaded.</p>
            <p>Put them together and storing is trivial: hash the name to get a box, drop it in. Finding it later is the same move: hash, then look in that one box. No searching anywhere, just arithmetic. Watch the names rain into their computed boxes; most boxes end up with one name, a few get crowded.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Tap your guess first, then watch the rain. A <strong>hash map</strong> is just two simple pieces working together.</p>
            <p>Piece one: an <strong>array of boxes</strong>, here called <strong>buckets</strong>. You met this row in the arrays lesson. Its superpower is that jumping to box number <code>i</code> is one step, whether there are 16 boxes or 16 million.</p>
            <p>Piece two: the recipe from the last step. Feed it any key, a name, a word, a number, and it answers with a box number. A good recipe spreads names out evenly, so no box gets swamped.</p>
            <p>Storing is hash-then-drop; finding is hash-then-open-that-one-box. Nothing is ever searched. But watch what the playback shows: 13 names, 16 boxes, and the recipe assigns boxes blindly, with no list of who took what. Whether every name still gets its own box is exactly what your guess is about.</p>
          </>
        ),
        rigorous: (
          <>
    ...
    },
    {
      ...
      takeaway: reg({
        ...
        intuitive: "Add, find, delete stay a few flat hops on average, because shared boxes stay short.",
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Insert, look up, delete: all <Term word="O(1)"><strong>O(1)</strong></Term> on average, so cost stays flat, ten keys or ten million. A <strong>collision</strong> is two keys landing in one box; we <strong>chain</strong> them (keep a small list in that box) so both fit. When it fills, we build a bigger one, which is slow but rare.</>,
            intuitive: <>Count the hops: add is hash-and-drop; find is hash, open one box, read a name or three; delete is the same trip. Ten names or ten million, the count doesn&rsquo;t change. Flat cost like that is written <Term word="O(1)"><code>O(1)</code></Term>. It&rsquo;s &ldquo;on average&rdquo; because of two wrinkles: a <strong>collision</strong> (two names, one box, chained into a tiny list) and the rare rebuild when the table fills.</>,
            rigorous: <>Put, get, delete each cost one hash + one jump + a chain walk of expected length &alpha; = n/m, the <strong>load factor</strong>. Held constant by resizing, that count is <Term word="O(1)"><code>O(1)</code></Term> expected. Worst case, every key in one bucket: <Term word="O(n)"><code>O(n)</code></Term>. A resize doubles <code>m</code> and rehashes all <code>n</code>, giving <Term word="amortized">amortized</Term> O(1) per insert.</>,
          }),
        },
        {
          left: 568, top: 300, width: 282, variant: "note",
          body: reg({
            base: <>Box 2 holds three names: <code>fawn, eli, june</code>. Looking one up still walks only that one short box, never the whole table.</>,
            rigorous: <>The worst case is real: keys engineered to share one bucket degrade lookups to <Term word="O(n)"><code>O(n)</code></Term>, which is why Python randomizes string hashing per process.</>,
          }),
        },
      ],
      detail: reg({
        base: (
          <>
            <p>The three everyday moves &mdash; <strong>insert</strong> (add a key), <strong>look up</strong> (find its value), and <strong>delete</strong> &mdash; all cost <strong><code>O(1)</code></strong> on average. <code>O(1)</code> means <em>constant time</em>: the cost stays flat whether the table holds ten keys or ten million, because hashing jumps straight to the right box. Notice the word <em>average</em>; that&rsquo;s the honest version, and here are the two wrinkles behind it.</p>
            <p>First, <strong>collisions</strong>. Sometimes two different keys hash to the <em>same</em> box. On the canvas, box 2 holds three names at once. We handle it by <strong>chaining</strong>: each box keeps a tiny list, and all of them live there side by side. To look one up, you hash to the box and then walk only that short list, never the whole table. As long as boxes stay short, this barely costs anything.</p>
            <p>Second, <strong>resizing</strong>. When the table gets too full, we build a bigger array and re-drop every key into it. That one step is <code>O(n)</code>, since it touches all <code>n</code> keys, so it&rsquo;s genuinely slow. But it happens so rarely that, spread across the many cheap inserts around it, the cost-per-insert <em>averaged out</em> is still tiny. That&rsquo;s the whole meaning of &ldquo;O(1) on average&rdquo;: the one slow step is rare enough to vanish in the average.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Price each everyday move by counting hops. <strong>Insert</strong>: hash the name, drop it in its box, two quick steps. <strong>Look up</strong>: hash, open that one box, read the name or the two or three sharing it. <strong>Delete</strong>: the same trip, then cross one out. None of those counts care how big the book is. Work that stays flat like that is <Term word="O(1)"><code>O(1)</code></Term>, &ldquo;constant time.&rdquo;</p>
            <p>Wrinkle one: <strong>collisions</strong>. Two names can land on the same box number. On the canvas, box 2 caught three. The fix is to <strong>chain</strong> them: that box keeps a tiny list, and a lookup walks only that list, never the whole table. Three names instead of one is barely slower; ten thousand in one box would be a disaster, which is exactly why the recipe spreads names evenly.</p>
            <p>Wrinkle two: <strong>the rebuild</strong>. When the table gets crowded, we make a bigger row of boxes and re-drop every name. That one moment really does touch all <code>n</code> names, so it is <Term word="O(n)"><code>O(n)</code></Term>, but it&rsquo;s rare, and spread across the thousands of cheap drops around it the average cost stays tiny. Cost smoothed out like that is called <Term word="amortized">amortized</Term>. That&rsquo;s the honest meaning of &ldquo;O(1) on average.&rdquo;</p>
          </>
        ),
        rigorous: (
          <>
            <p><strong>Exact costs.</strong> Put, get, delete: one hash + one index + an expected &alpha; = n/m chain walk, so expected <Term word="O(1)"><code>O(1)</code></Term> under uniform hashing with &alpha; bounded (CPython resizes around two-thirds full). Worst case &mdash; all keys in one bucket &mdash; is <Term word="O(n)"><code>O(n)</code></Term>. A resize rehashes all <code>n</code> keys into <code>2m</code> buckets: O(n) once, <Term word="amortized">amortized</Term> O(1) per insert. Iterating everything is O(n); any order you observe (CPython preserves insertion order) is an implementation promise, not a hash-table property.</p>
            <p><strong>Edges.</strong> A miss walks the full chain before <code>KeyError</code>: absence costs as much as a hit. The empty map still computes a slot; the bucket is just empty. Keys must be hashable, hence immutable: a <code>list</code> can&rsquo;t key a dict, since mutating it would change its slot and orphan the entry. Equal keys must hash equal, the contract that makes overwrite-on-put correct. And adversarial keys forcing one bucket are why string hashes are randomized per process.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: <>Reach for a hash map whenever you&rsquo;d say &ldquo;given X, find Y&rdquo;: counting how often each word appears, saving (<strong>caching</strong>) a slow result to reuse, removing duplicates, joining two datasets. The one thing it can&rsquo;t do: keep order, or answer &ldquo;all keys between A and M.&rdquo;</>,
      }],
      detail: (
        <>
          <p>The simplest tell: reach for a hash map any time you can phrase the job as <strong>&ldquo;given X, find Y&rdquo;</strong>, such as given a name, find a number; given a word, find its count. If you have a key and want the thing attached to it, this is the tool.</p>
          <p>That covers an enormous amount of real work:</p>
          <ul>
            <li><strong>Counting</strong>: how many times each word appears in a document (the word is the key, the count is the value).</li>
            <li><strong>Caching</strong>: saving the result of a slow computation under its inputs, so the next time the same inputs show up you reuse the answer instead of redoing the work.</li>
            <li><strong>Deduplicating</strong>: remembering what you&rsquo;ve already seen so you can drop repeats from a stream.</li>
            <li><strong>Joining</strong>: matching up two datasets on a shared field, like stitching orders to customers by id.</li>
          </ul>
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>That&rsquo;s the named pattern: hash map, hash table, <strong>dictionary</strong> (a store of key&rarr;value pairs), the same idea everywhere. Python&rsquo;s <code>dict</code> is one; so are JavaScript&rsquo;s <code>Map</code> and Java&rsquo;s <code>HashMap</code>. The principle underneath: spend memory to never search.</>,
          intuitive: <>The thing you built has a name &mdash; several, actually: <strong>hash map</strong>, hash table, <strong>dictionary</strong>. Python&rsquo;s <code>dict</code> is one; so is JavaScript&rsquo;s <code>Map</code>. And the deal underneath is idea 5 of 7, <strong>trade space for time</strong>: spend a roomy row of boxes so finding anything is one hop, never a search.</>,
          rigorous: <>Hash map / hash table / dictionary / associative array: one structure. Python&rsquo;s <code>dict</code>, Java&rsquo;s <code>HashMap</code>, Go&rsquo;s <code>map</code>: same cost model. It is the full statement of <strong>trade-space-for-time</strong>, idea 5 of 7: memory spent on sparse buckets buys O(1)-average membership, cost decoupled from <code>n</code>.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>Different languages give this same idea different names: <em>hash map</em>, <em>hash table</em>, <strong>dictionary</strong>, <em>associative array</em>. They all mean the one thing you just built: a store of <strong>key&rarr;value pairs</strong>, where you hand it a key and it hands you back the value in one hop.</p>
            <p>You already use these constantly. Python&rsquo;s <code>dict</code> is a hash map. So is JavaScript&rsquo;s <code>Map</code> (and, loosely, its plain objects), Java&rsquo;s <code>HashMap</code>, and Go&rsquo;s <code>map</code>. It&rsquo;s the workhorse data structure of everyday programming, the thing you reach for without even thinking.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 5 of 7, trade space for time:</strong> spend memory on a clever filing system and &ldquo;is it here?&rdquo; stops depending on size.
            </div>
          </>
        ),
        intuitive: (
          <>
            <p>Different languages, one idea: <em>hash map</em>, <em>hash table</em>, <strong>dictionary</strong>, <em>associative array</em>. All of them are the machine you just built: a store of <strong>key&rarr;value pairs</strong>, where you hand over a key and get its value back in a hop.</p>
            <p>You&rsquo;ll use this constantly without ceremony: Python&rsquo;s <code>dict</code>, JavaScript&rsquo;s <code>Map</code>, Java&rsquo;s <code>HashMap</code>, Go&rsquo;s <code>map</code>. It&rsquo;s the workhorse of everyday code, the thing programmers reach for without thinking.</p>
            <p>And remember where the magic came from. In arrays you paid a strict layout to make <em>positions</em> instant &mdash; a seed. Here the seed flowered: we set aside a whole roomy row of boxes, memory we didn&rsquo;t strictly need, so that <code>phone[&ldquo;alice&rdquo;]</code> is one hop instead of ten thousand reads.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The big idea, claimed (5 of 7), trade space for time:</strong> write it down in the right place and you can look it up instantly, and size stops mattering.
            </div>
          </>
        ),
        rigorous: (
          <>
            <p>Hash table: a bucket array addressed by a hash of the key. Python&rsquo;s <code>dict</code> (open addressing, randomized string hashes, resize near two-thirds load), Java&rsquo;s <code>HashMap</code> (chaining, long chains treeified), Go&rsquo;s <code>map</code>: implementations differ, the cost model doesn&rsquo;t. Expected O(1) put/get/delete, O(n) worst case, no order, no range queries.</p>
            <p><strong>Trade space for time, idea 5 of 7</strong>, here as the theorem, not the seed: arrays bought O(1) <em>position</em> with layout; the hash map spends sparse buckets to buy O(1) <em>membership</em>. Memoization and caching replay this exact trade.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 5 of 7, trade space for time:</strong> spend memory so the answer is looked up, never searched for, with cost decoupled from n.
            </div>
          </>
        ),
      }),
