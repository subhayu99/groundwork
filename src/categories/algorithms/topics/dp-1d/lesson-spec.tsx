  ...
  beats: [
    {
      ...
      detail: reg({
        base: (
          <>
            <p>Picture standing at the bottom of a staircase with <strong>8 steps</strong>. Every move, you may hop up either <strong>1 step</strong> or <strong>2 steps</strong>, your choice each time. The question is simple to ask: how many <em>different</em> routes get you all the way to the top?</p>
        ...
        intuitive: (
          <>
            <p>Take the staircase one size at a time. A 1-step staircase: one route (a single small hop). A 2-step staircase: two routes, hop-hop or one big 2-hop. A 3-step staircase: three routes, 1+1+1, 1 then 2, or 2 then 1.</p>
            <p>Try 4 steps in your head and it already gets slippery: routes blur together and it&rsquo;s easy to count one twice. By 8 steps there are dozens of routes, and the count keeps climbing fast as the staircase grows.</p>
            <p>Patience won&rsquo;t do it. We need a method that can&rsquo;t miscount, and the whole lesson is about finding the laziest correct one.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      detail: (
        <>
          ...
          <p>Here&rsquo;s the trap. To get <code>ways(8)</code> we ask for <code>ways(7)</code> and <code>ways(6)</code>. But <code>ways(7)</code> <em>also</em> needs <code>ways(6)</code>, so we compute <code>ways(6)</code> twice. Further down it&rsquo;s far worse: small cases like <code>ways(2)</code> get re-computed dozens of times. The work isn&rsquo;t hard. It&rsquo;s repeated.</p>
        </>
      ),
    ...
    },
    {
      ...
      connector: reg({
        ...
        structured: "Only the last hop matters, so ways(n) = ways(n−1) + ways(n−2); run naively, ways(8) makes 67 calls for just 34 routes.",
      }),
      ...
      panels: [
        {
          ...
          body: reg({
            ...
            intuitive: <>This branching picture of who-asks-whom is called a <strong>tree</strong>; each circle is one little ask, &ldquo;ways up k stairs?&rdquo; The same numbers keep reappearing in different branches. Toggle <strong>remember answers</strong>: the second time an ask comes back, we hand over the answer we wrote down, and the green circles stop being work.</>,
          }),
        },
        {
          left: 560, top: 348, width: 280, variant: "note",
          body: <><strong className="text-[var(--accent-ink)]">The instinct:</strong> every sub-question has exactly one true answer. Computing it twice is pure waste, so write it down once.</>,
        },
      ],
      detail: reg({
        base: (
          <>
            ...
            <p>Now toggle <strong>remember answers</strong>. The first time we compute some <code>ways(k)</code>, we jot the result down. The next time that exact call appears, we don&rsquo;t redo it. We just hand back the number we already wrote. On the diagram, those repeat calls light up green and effectively disappear, and the tree collapses to a fraction of its size.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The instinct:</strong> every sub-question has exactly one true answer. Computing it twice is pure waste, so write it down the first time, and reuse it forever after.
            </div>
          </>
        ),
        intuitive: (
          <>
            <p>The picture is a <strong>tree</strong> of questions, a branching map of who-asks-whom. The top circle is the big ask, &ldquo;ways up 6?&rdquo; It sets off two smaller asks, &ldquo;ways up 5?&rdquo; and &ldquo;ways up 4?&rdquo;, and each of those sets off two more, all the way down. Now scan across the rows: the <em>same numbers</em> keep reappearing in different branches. Every repeat is the computer re-answering a question it has already answered.</p>
            <p>Hit <strong>remember answers</strong>. New deal: the first time a question gets answered, the answer is written down; any later copy of that question is handed the note instead of being re-worked. On the picture, every repeat lights up green, looked up rather than redone, and the real work shrinks to a handful of circles.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The instinct:</strong> a question like &ldquo;ways up 4?&rdquo; has exactly one true answer. Working it out twice buys nothing, so write it down once, and reuse it forever after.
            </div>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Top-down: keep the rule, add a notebook: check if it&rsquo;s written down before working, store it after. Bottom-up: drop recursion, build from the smallest case up. Since each step only needs the last two answers, the code keeps just two rolling variables: a, b = 1, 1, then a, b = b, a + b each step.</>,
          intuitive: <>Two builds, same result. Top-down: keep the self-calling rule, but peek in the notebook before working and write the answer in after. Bottom-up: skip self-calls entirely, filling a row of answers starting from the easy ends, so each new slot just adds the two before it. And since only the last two matter, two little boxes are all the memory you really need.</>,
          rigorous: <>Top-down: cache the bare recursion: hit the cache, or compute once and store. Bottom-up: <code>dp[0] = dp[1] = 1</code>, then <code>dp[i] = dp[i&minus;1] + dp[i&minus;2]</code> in increasing <code>i</code>, so every dependency is final when read. Only two values are ever live, so the table contracts to a rolling pair: <code>a, b = b, a + b</code>.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p><strong>Top-down (remember as you go).</strong> Keep the recursive rule exactly as it is, but give it a notebook: a lookup table that maps each <code>k</code> to its answer. Before doing any work for <code>ways(k)</code>, peek in the notebook; if the answer&rsquo;s already there, hand it straight back. Otherwise compute it once and write it down. No sub-question is ever solved twice.</p>
            ...
            <p>And notice: the bottom-up form only ever looks at the last two values, so you don&rsquo;t even need the whole array. Two variables are enough.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The principle of information reuse:</strong> the same sub-question never gets solved twice. The whole speed-up comes from one move: <em>write it down so you can look it up.</em>
            </div>
          </>
        ),
        intuitive: (
          <>
            <p><strong>Build one: remember as you go.</strong> Keep the rule from before exactly as it is, and give it a notebook. Before working anything out, peek: is &ldquo;ways up k?&rdquo; already written down? Hand it back. Not there? Work it out once, write it in. No question ever gets answered twice.</p>
            <p><strong>Build two: fill a row.</strong> Forget self-calling. Draw a row of slots where slot <code>i</code> will hold &ldquo;ways up <code>i</code> stairs.&rdquo; The two easiest slots are free: 1 way up 0 stairs (stand still) and 1 way up 1 stair. Now move right, one slot at a time, filling each as the-slot-before plus the-slot-two-before. Whenever you arrive at a slot, the two answers it needs are already sitting there.</p>
            <p>One more squeeze: filling a slot only ever uses the <em>last two</em> answers, so you don&rsquo;t even need the row. Two little boxes, shuffled forward each step, carry everything.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The principle, reuse what you know:</strong> no small question is ever answered twice. The entire speed-up is one move: <em>write it down so you can look it up.</em>
            </div>
          </>
        ),
        rigorous: (
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Naive recursion roughly doubles its work per extra step: ways(40) calls itself about 330 million times. Filling each table slot once is <Term word="O(n)"><strong>O(n)</strong></Term> (work grows in step with the n stairs). Keeping just the last two values is <Term word="O(1)"><strong>O(1)</strong></Term>, a fixed cost at any height.</>,
          intuitive: <>Run blind, every extra stair roughly <em>doubles</em> the work. By 40 stairs that&rsquo;s hundreds of millions of repeats. With answers written down, each slot costs one small addition. Make your prediction, then watch the row prove it: that once-per-stair work is called <Term word="O(n)"><code>O(n)</code></Term>, and carrying only the last two answers is <Term word="O(1)"><code>O(1)</code></Term> memory &mdash; the same tiny pocketful at any height.</>,
          rigorous: <>Naive: the call tree doubles per level, so ways(40) is about 330 million calls. Fixed: the loop runs <code>i = 2&hellip;n</code>, one addition each; once the row confirms the count, that&rsquo;s <Term word="O(n)"><code>O(n)</code></Term> time. The rolling pair is <Term word="O(1)"><code>O(1)</code></Term> space; the memo table instead pays <Term word="O(n)"><code>O(n)</code></Term> space plus recursion depth.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p><strong>Naive recursion</strong> roughly <em>doubles</em> its work for every extra step, because each call spawns two more. By the time you ask for <code>ways(40)</code> the function calls itself about 330 million times: minutes of computing for a question with a one-line answer.</p>
            <p><strong>Either fixed version</strong>, remember-as-you-go or fill the table, computes each <code>i</code> from 0 up to <code>n</code> exactly once, doing a single addition each time. That total cost is <code>O(n)</code> (&ldquo;order n&rdquo;): the work grows in lock-step with how many stairs there are. Watch the row above fill itself, one slot per beat, left to right.</p>
            <p>And memory: the top-down notebook holds <code>n+1</code> answers, but the bottom-up form only needs the last two numbers as it goes. That&rsquo;s <code>O(1)</code> (&ldquo;order one&rdquo;), a fixed, tiny amount no matter how tall the staircase gets.</p>
          </>
        ),
        intuitive: (
          <>
            <p>First, feel the blow-up. Run blind, every extra stair roughly doubles the routes the computer re-walks: 8 stairs took 67 calls; 40 stairs would take about 330 million. The questions aren&rsquo;t hard. They&rsquo;re just asked over and over.</p>
            <p>Now count the fixed version on the row above: slot 2, slot 3, &hellip; slot 8, each filled <em>once</em>, with one addition. Nine slots, nine little jobs, done. Work that grows in step with the number of stairs is written <Term word="O(n)"><code>O(n)</code></Term>. Tap the chip; it just means &ldquo;twice the stairs, twice the additions.&rdquo;</p>
            <p>Memory is even better. The notebook version keeps every answer, one per stair. The row version only ever peeks at the <em>last two</em> slots, so it carries two numbers in its pocket no matter how tall the staircase. That fixed pocketful is <Term word="O(1)"><code>O(1)</code></Term>.</p>
        ...
        rigorous: (
          <>
            <p><strong>Exact costs.</strong> Naive recursion satisfies T(n) = T(n&minus;1) + T(n&minus;2) + O(1): exponential growth (~1.618&#8319;); ways(40) is about 330 million calls. Memoized or tabulated: each of the n&minus;1 loop indices does one addition, giving O(n) time. Space: the memo table holds n+1 entries plus n frames of recursion depth; the rolling pair is O(1).</p>
            <p><strong>Edges.</strong> <code>n = 0</code> returns 1, the empty climb counts as one way, the convention the recurrence needs; <code>n = 1</code> hits the same base check, and the loop body never runs for either. The memoized form recurses n deep, so Python&rsquo;s ~1000-frame recursion limit bites near n &asymp; 1000; the loop form can&rsquo;t. Counts grow fast (ways(500) has 105 digits): Python&rsquo;s ints stay exact, while fixed-width languages overflow 64 bits past n = 91.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      detail: (
        <>
          ...
          <p>Two ingredients are required. First, the answer must break into <em>overlapping</em> parts: if every sub-question were different, there&rsquo;d be nothing to reuse. Second, each sub-question must have one fixed answer that never changes, so it&rsquo;s safe to write down once and trust forever.</p>
        </>
      ),
    ...
    },
    {
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>That&rsquo;s the name, misleading since nothing is &ldquo;dynamic.&rdquo; It just means: solve overlapping sub-questions once, write them down, look them up.</>,
            intuitive: <>That&rsquo;s the name, an odd one, since nothing here is &ldquo;dynamic.&rdquo; The move is exactly what you did: solve each small question once, write it down, look it up ever after.</>,
            rigorous: <>The name (Bellman&rsquo;s, deliberately vague) covers exactly this: overlapping subproblems, each with one fixed answer, solved once, memoized or tabulated.</>,
          }),
        },
        {
      ...
      detail: reg({
        base: (
          <>
            <p>That&rsquo;s the name: <strong>Dynamic Programming</strong>. The dramatic phrase is misleading, since nothing about it is &ldquo;dynamic&rdquo; in any everyday sense. It simply means: <em>solve overlapping sub-questions once, write the answers down, and look them up.</em></p>
        ...
        intuitive: (
          <>
            ...
            <ul>
              ...
              <li>there&rsquo;s a small rule answering the big case from smaller cases: like ways(n) = ways(n&minus;1) + ways(n&minus;2)</li>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The first big idea (1 of 7), reuse what you already know:</strong> you just watched it collapse a mountain of repeats into one quick pass. Never figure out the same thing twice.
            </div>
          </>
        ),
        rigorous: (
          <>
            <p><strong>Dynamic programming:</strong> overlapping subproblems + one fixed answer per subproblem (which is what makes caching sound). Solve each once: memoize the recursion, or tabulate in dependency order; reconstruct from the table when the problem wants the route, not just the count.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 1 of 7, information reuse:</strong> cache every subproblem&rsquo;s answer; the exponential tree collapses to one linear pass.
            </div>
          </>
        ),
      }),
