  ...
  return (
    <g>
      <text x={VW / 2} y={214} textAnchor="middle" className="font-mono select-none" style={{ fontSize: 12, fill: "var(--text-faint)" }}>
        page by page could take up to 1,000 looks; now price the halving way
      </text>
...
export const binarySearchLesson: LessonSpec = {
  ...
  bridgeFrom: reg({
    base: "An array hands you any position in one step. Add sorted order, and one look rules out half the row.",
  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>You&rsquo;re holding a sorted phone book, a thousand names in order. You wouldn&rsquo;t start at page one. You&rsquo;d flip to the <strong>middle</strong>, see if you&rsquo;ve gone too far, and throw away half. Then again.</>,
          intuitive: <>Picture a real phone book in your lap: a thousand names, already in A-to-Z order. You need exactly one. Nobody starts at page one. You open the <strong>middle</strong>, glance once, and you know which half the name lives in. The other half? You never touch it.</>,
      ...
      detail: reg({
        base: (
          <>
            ...
            <p>You flip to the <strong>middle</strong> and look. If that page comes <em>after</em> the name you want, the whole back half is useless, so ignore it. If it comes <em>before</em>, the front half is gone. Either way, one glance throws away half the book. Then you repeat the same move on whatever&rsquo;s left.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Here&rsquo;s the game. A phone book holds a thousand names in A-to-Z order, and you need to find just one. Reading from page one would work, eventually. Your hands already know a better way.</p>
            <p>Open the book near the <strong>middle</strong> and read one name. Does the name you want come earlier in the alphabet? Then the entire back half is useless, so let it go. Does it come later? The front half is gone instead. One glance, five hundred pages thrown away. Now play the same move on the half that&rsquo;s left.</p>
          </>
        ),
        rigorous: (
          <>
    ...
    },
    {
      ...
      takeaway: reg({
        base: "Reading page by page is O(n), and it wastes the sorted order.",
      ...
      panels: [{
        ...
        body: reg({
          base: <>The plain way: page 1, page 2, page 3 &hellip; up to a thousand checks. But the book is <strong>sorted</strong>, and that barely helped. Each page only said &ldquo;not here,&rdquo; never <em>how far</em> off you were.</>,
          intuitive: <>Try the plain way: page 1, page 2, page 3 &hellip; if the name sits near the back, that&rsquo;s a thousand looks. And here&rsquo;s the itch: the book is in perfect order, yet you never used that. Every page just said &ldquo;nope,&rdquo; never &ldquo;getting warmer.&rdquo;</>,
          rigorous: <>The baseline: probe indices 0, 1, 2, &hellip; until equality. Worst case n probes, O(n), and a miss always costs all n. It needs no precondition at all, which is the indictment: on sorted input it runs no faster.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>The obvious method: read every page in order, page 1, page 2, page 3, until you hit the name. For a thousand pages that&rsquo;s up to a thousand checks. This is a <strong>linear scan</strong>, and its cost is <Term word="O(n)"><code>O(n)</code></Term> (&ldquo;order n&rdquo;): the work grows in step with the number of items <code>n</code>. Double the book, double the worst case.</p>
            <p>The frustrating part: the book is already sorted, and the scan barely uses that. Each page you flip past only says &ldquo;not here,&rdquo; never <em>how far away</em> the name still is. Could a single comparison tell us where it <em>is</em>, not just where it isn&rsquo;t?</p>
          </>
        ),
        intuitive: (
          <>
            <p>The obvious method: start at page one and check each page in turn until you hit the name. A thousand pages can mean a thousand looks. Double the book, double the looks: the effort grows in step with the size of the pile. (That&rsquo;s all the symbol <Term word="O(n)"><code>O(n)</code></Term> means, when you meet it.)</p>
            <p>Now the part that should bother you. The book is already in perfect order, and the scan never once used it. Every page you flipped past said only &ldquo;not here.&rdquo; Never &ldquo;you&rsquo;re close,&rdquo; never &ldquo;way past it.&rdquo; Could one look instead tell you which <em>direction</em> the name is?</p>
          </>
        ),
        rigorous: (
          <>
            <p>The scan probes indices in order until equality: worst case n probes, O(n); a miss costs exactly n. It assumes nothing about the input, which is the waste, since it runs identically on sorted data.</p>
            <p>Per-probe information is the tell: an equality test eliminates one element; a three-way comparison on sorted data eliminates an entire prefix or suffix. Same probe cost, strictly more pruning. The next beat buys the stronger test.</p>
          </>
        ),
      }),
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Click any page. Land on a number bigger than 27? Because the book is sorted, <em>everything to its right is bigger too</em>: all gone, in one look. Smaller? the left half goes. That&rsquo;s the whole trick.</>,
            intuitive: <>Click any number in the row. Say you land on one bigger than 27. The row is sorted, so everything to its right is bigger still. None of those can be 27. They all go dark in one look. Land on a smaller one? The left side goes dark instead. That&rsquo;s the whole trick.</>,
            rigorous: <>Probe any index <code>i</code>. If <code>arr[i] &gt; 27</code>, sortedness gives <code>arr[j] &gt; 27</code> for every <code>j &gt; i</code>: the suffix dies unexamined. If <code>arr[i] &lt; 27</code>, the prefix dies. The dimmed cells are exactly the eliminated set.</>,
          }),
        },
        {
          ...
          body: reg({
            ...
            rigorous: <><strong className="text-[var(--accent-ink)]">The question:</strong> each probe halves the live set. How many probes take n down to 1?</>,
          }),
        },
      ],
      detail: reg({
        base: (
          <>
            <p>Click any cell to guess. Because the array is sorted, the moment you land on a number <em>bigger</em> than the target, every number to its right is bigger too, so they all vanish at once, even though you never looked at them one by one. Land on a <em>smaller</em> number and the whole left side disappears instead.</p>
            <p>One comparison eliminates half of everything still in play. That is the entire trick, and exactly what the linear scan was throwing away.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Go on &mdash; click any number in the row. Suppose you hit one bigger than 27. The row is in order, so every number to its right must be bigger too. You never looked at them, yet you <em>know</em> none of them is 27, so they all go dark at once. Hit a smaller number and the whole left side goes dark instead.</p>
        ...
        rigorous: (
          <>
            <p>The pruning lemma: probing <code>i</code> with <code>arr[i] &lt; target</code> eliminates indices <code>0..i</code>; with <code>arr[i] &gt; target</code>, it eliminates <code>i..n-1</code>. Nothing in the discarded side is examined: sortedness certifies it wholesale. Click cells and watch the eliminated set (the dim cells) grow.</p>
    ...
    },
    {
      ...
      detail: reg({
        base: (
          <>
            <p>Hold two markers: <code>lo</code> at the start and <code>hi</code> at the end of the part that could still contain the target. So the answer, if it exists, is somewhere in <code>[lo, hi]</code>.</p>
            ...
            <ul>
              <li><Term word="arr[i]"><code>arr[mid]</code></Term> <code>== target</code>: done, return <code>mid</code>.</li>
              <li><code>arr[mid] &lt; target</code>: the answer is to the right, move <code>lo = mid + 1</code>.</li>
              <li><code>arr[mid] &gt; target</code>: the answer is to the left, move <code>hi = mid - 1</code>.</li>
            </ul>
            <p>Stop when <code>lo &gt; hi</code>: the search space is empty, so the target isn&rsquo;t there.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The principle, search space pruning:</strong> every comparison eliminates an entire side, not just one element.
            </div>
          </>
        ),
        intuitive: (
          <>
            <p>Give the instinct a uniform a computer can wear. Keep two markers: <Term word="lo"><code>lo</code></Term> marks the first spot still worth checking, <Term word="hi"><code>hi</code></Term> the last. Between them is the only place 27 can still hide. Watch the dark cells close in from both ends as the run plays.</p>
            ...
            <ul>
              <li>It&rsquo;s 27. Done, that&rsquo;s the answer.</li>
              <li>It&rsquo;s smaller, so 27 must be further right. Move the left marker just past the middle.</li>
              <li>It&rsquo;s bigger, so 27 must be further left. Move the right marker just before the middle.</li>
            </ul>
            <p>If the markers ever cross, there&rsquo;s nowhere left to hide: 27 isn&rsquo;t in the book.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The big idea:</strong> every look throws away a whole side, never just one page.
            </div>
          </>
        ),
        rigorous: (
          <>
            ...
            <p><strong>Cost.</strong> The window at least halves per iteration: ⌈log&#8322;(n+1)⌉ comparisons worst case, exactly 4 for this n = 15.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Pitfalls:</strong> the ±1 is load-bearing, since <code>lo = mid</code> loops forever on a two-element window. And <code>(lo + hi) // 2</code> overflows fixed-width ints in C/Java, so write <code>lo + (hi - lo) // 2</code> there; Python&rsquo;s arbitrary-precision ints make the plain form safe.
            </div>
          </>
        ),
      }),
    ...
    },
    {
      ...
      connector: reg({
        ...
        rigorous: "Count it: the window halves per probe, so the recurrence is T(n) = T(n/2) + 1.",
      }),
      actionLabel: reg({
      ...
      panels: [{
        ...
        body: reg({
          base: <>Checking a thousand pages one by one: up to 1,000 looks. Halving: about 10. A million? one-by-one needs a million; halving needs about 20. That gap is why sorted data everywhere, from search to database indexes, runs on this.</>,
          ...
          rigorous: <>The cascade halves 1,000 down to 1. Worst case is ⌈log&#8322;(n+1)⌉ comparisons: 10 for 10³, 20 for 10⁶, 30 for 10⁹, and doubling n adds one probe. That additive growth is why B-tree indexes and <code>bisect</code>-style lookups stay fast at scale.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>Compare the costs head to head. A thousand items one by one: up to 1,000 looks. Halving repeatedly: about 10. Bump it to a million, and one-by-one needs a million, halving needs about 20.</p>
            <p>That &ldquo;about 10/20&rdquo; is <code>log&#8322;</code> (&ldquo;log base two&rdquo;): how many times you can halve a pile before one item remains. Big-O calls it <Term word="O(log n)"><code>O(log n)</code></Term>, work that grows by just one extra step each time the data <em>doubles</em>. That gap is why sorted-data tools everywhere lean on it, from Python&rsquo;s <code>bisect</code> to the index pages (B-trees) your database uses to find a row fast.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Follow the cascade: 1,000 → 500 → 250 → 125 &hellip; down to 1. Count the hops, about ten. So a thousand-page book takes about ten looks instead of up to a thousand. A million names? About twenty looks. And doubling the book again adds just <em>one</em> extra look.</p>
        ...
        rigorous: (
          <>
            ...
            <p>In the wild: <code>bisect</code> / <code>lower_bound</code> on sorted arrays; B-trees, the high-fanout cousin, behind database indexes, trading log&#8322; for log<sub>fanout</sub> to match disk pages.</p>
          </>
        ),
      }),
      codeLabels: ["loop", "mid"],
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>The phone-book version finds an exact value. The deeper one finds the <strong>boundary</strong> between &ldquo;too small&rdquo; and &ldquo;big enough,&rdquo; e.g. &ldquo;smallest ship that finishes in 14 days?&rdquo; No list at all, but bigger is always easier, so guess the middle and halve.</>,
          intuitive: <>The phone book found an exact name. The bigger idea finds a <strong>tipping point</strong>. Ask: &ldquo;what&rsquo;s the smallest ship that can deliver everything in 14 days?&rdquo; No list anywhere, but a bigger ship is never worse, so the answers line up like the boxes above: no, no, no &hellip; yes, yes. Guess a middle size, ask &ldquo;does it work?&rdquo;, throw half away.</>,
          rigorous: <>Replace <code>arr[mid] == target</code> with any predicate P(x) that flips false → true once over an ordered domain: the boxes above are its truth table. Binary search returns the boundary: the least x with P(x) true. &ldquo;Smallest ship that finishes in 14 days&rdquo;: feasibility is monotone in capacity, so bisect capacity, no array required.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            ...
            <p>But the answers line up in one direction: the bigger the ship, the easier the job, so once a size works, every larger size works too. That one-way property is called <Term word="monotonicity"><strong>monotonicity</strong></Term>, and it&rsquo;s all binary search needs. Guess the middle, ask &ldquo;does this work?&rdquo;, throw away half. Small to big, easy to hard, no to yes, one check halves it.</p>
          </>
        ),
        intuitive: (
          <>
            <p>So far we searched a book. The deeper trick searches for a <strong>tipping point</strong>. Try this question: &ldquo;What&rsquo;s the smallest ship that can finish all the deliveries in 14 days?&rdquo; There&rsquo;s no list of pages anywhere, just ship sizes you could imagine trying.</p>
            <p>Here&rsquo;s what saves us: a bigger ship is never worse. Once some size works, every bigger size works too, so the answers line up exactly like the row above: no, no, no, then yes forever. (The fancy word for &ldquo;answers that only flip one way&rdquo; is <Term word="monotonicity"><strong>monotonicity</strong></Term>.) Play the same game: try a middle size. Works? Look smaller. Doesn&rsquo;t? Look bigger. Half the sizes vanish per question.</p>
          </>
        ),
        rigorous: (
          <>
            <p>The array was never essential. Sufficient: a predicate P over an ordered domain that flips false → true exactly once, monotone, with boundary b. Bisect the domain: ⌈log&#8322;(range)⌉ probes, each one evaluation of P, returning b (the least x with P(x) true).</p>
    ...
    },
    {
      ...
      detail: reg({
        base: (
          <>
            <p>That&rsquo;s the name: <strong>binary search</strong>. Two conventions are worth knowing, because mixing them up is the classic off-by-one bug. <em>Closed bounds</em>: loop while <code>lo &lt;= hi</code>, with <code>hi</code> = the last index (what you just watched). Or <em>half-open</em>: loop while <code>lo &lt; hi</code>, with <code>hi</code> = one-past-the-end. Both are correct; pick one and stop second-guessing.</p>
        ...
        intuitive: (
          <>
            <p>You just derived a famous algorithm: <strong>binary search</strong>. (&ldquo;Binary&rdquo; = two: every look cuts what&rsquo;s left in two.) One habit worth copying from the run you watched: decide exactly what your two markers mean, here the first and last spots still in play, and never let a move break that meaning. That&rsquo;s what keeps every marker move honest.</p>
            <p>Reach for it when you see:</p>
            <ul>
