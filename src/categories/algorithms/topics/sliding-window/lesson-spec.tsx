...
export const slidingWindowLesson: LessonSpec = {
  ...
  bridgeFrom: reg({
    ...
    rigorous: "arr[i] is O(1) — access is free; the only cost left is redundant arithmetic across overlapping windows.",
  }),
  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Here&rsquo;s a row of ten numbers. A friend points at the first three &mdash; we&rsquo;ll call that frame around them a <strong>window</strong> &mdash; and asks &ldquo;what do these add up to?&rdquo; Then slides it one cell right and asks again. Eight times.</>,
          structured: <>A row of ten numbers; a three-cell frame &mdash; a <strong>window</strong> &mdash; slides one cell at a time: eight stops, eight sums. The plain answer re-adds all three at every stop &mdash; 3 &times; 8 = 24 additions &mdash; even though neighbouring windows share two of their three numbers.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            <p>Picture a paper frame lying on the row, covering exactly three numbers. Your friend only ever asks one thing: <em>&ldquo;what do the covered numbers add up to?&rdquo;</em> That frame is our <strong>window</strong> &mdash; just a marked-off stretch of the row.</p>
        ...
        structured: (
          <>
            ...
            <p>The from-scratch baseline adds three numbers per window &mdash; 3 &times; 8 = 24 additions &mdash; and feels fine until you notice that adjacent windows overlap in two of their three cells, so it keeps re-adding numbers it has already added. That overlap is the lever this whole lesson pulls.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Your turn. Tap a cell beside the window &mdash; or use the <strong>&ldquo;&lsaquo; left&rdquo;</strong> / <strong>&ldquo;right &rsaquo;&rdquo;</strong> buttons under the row &mdash; to nudge it one step. Don&rsquo;t do mental math; just watch the cells. One number drops out of the left edge, one new number joins on the right, and the middle one doesn&rsquo;t move at all.</>,
            intuitive: <>First, commit to a guess at the <strong>predict</strong> card &mdash; one tap. Then the window is yours: tap a cell beside it, or use the <strong>&ldquo;&lsaquo; left&rdquo;</strong> / <strong>&ldquo;right &rsaquo;&rdquo;</strong> buttons under the row, and nudge it one step at a time. No mental math &mdash; just watch which cells change and which sit still.</>,
            structured: <>Commit at the <strong>predict</strong> card first &mdash; one tap. Then nudge the window a step at a time &mdash; tap a cell beside it, or use the <strong>&ldquo;&lsaquo; left&rdquo;</strong> / <strong>&ldquo;right &rsaquo;&rdquo;</strong> buttons &mdash; and track which cells actually change roles as it moves.</>,
          }),
        },
        {
      ...
      detail: reg({
        ...
        intuitive: (
          <>
            <p>Answer the predict question beside the row before you move anything &mdash; commit with one tap. Then the window is live.</p>
        ...
        structured: (
          <>
            <p>Commit at the predict card, then drag: tap a cell on either side of the window, or step it with the buttons. The tones name the roles &mdash; the old left edge exits, a new right edge enters, and the cells between keep their places.</p>
    ...
    },
    {
      ...
      takeaway: reg({
        ...
        rigorous: "Invariant: window_sum equals the current window's sum — the edge update keeps it true in two ops.",
      }),
      visual: <FrozenSlide />,
      panels: [{
        ...
        body: reg({
          base: <>Two cells change per slide. Keep a running total &mdash; call it <code>window_sum</code>. The new total is the old total, minus the number that <strong>left</strong>, plus the number that <strong>entered</strong>. Just two steps, any window width. <span className="text-[var(--accent-ink)]">Add each number once, then reuse the total forever.</span></>,
          intuitive: <>You saw it: two cells change, the rest sit still. So keep one number alive as you go &mdash; the total of whatever the window covers right now, a <Term word="accumulator">running total</Term> called <code>window_sum</code>. Each slide: take away the number that <strong>left</strong>, add the number that <strong>joined</strong>. Two tiny steps &mdash; and the middle is never re-added.</>,
          rigorous: <>Maintain <code>window_sum</code> under the invariant: <em>it equals the sum of the cells currently in the window</em>. Consecutive windows differ in exactly two cells, so <code>window_sum &minus; leaver + newcomer</code> re-establishes the invariant &mdash; two operations per slide, independent of the width <code>k</code>.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            ...
            <p>So carry one number with you &mdash; the total of what the window covers right now. A carried number like that is called a <Term word="accumulator">running total</Term>: you repair it instead of rebuilding it. One slide = take away the leaver, add the joiner. That&rsquo;s the whole move.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The trick in one line:</strong> pay to add each number once, then reuse that work forever &mdash; never re-figure-out what you already figured out.
            </div>
          </>
        ),
        rigorous: (
          <>
            ...
            <p><strong>The mechanism is information reuse:</strong> each element enters the sum once and leaves it once; the interior is never re-touched. Two operations per slide, whatever <code>k</code> is &mdash; the count the next beat totals up.</p>
          </>
        ),
      }),
      arrows: [
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          ...
          intuitive: <>Read the race: the obvious way pays <code>k</code> additions per slide (here 3), the instinct way pays 2 &mdash; totals 24 against 17. Now grow the window to 100: the obvious way pays 100 per slide, the instinct way still pays 2. About fifty times the work, gone.</>,
          rigorous: <>Total it: setup pays <code>k</code>, then <code>n &minus; k</code> slides at 2 ops &mdash; <code>k + 2(n &minus; k)</code> = 17 here. Rebuilding pays <code>k</code> per window &mdash; <code>k(n &minus; k + 1)</code> = 24. Counted, now named: the pass is <Term word="O(n)"><code>O(n)</code></Term> total, independent of <code>k</code>; the rebuild is O(n·k).</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            <p>Count the race honestly. Eight windows: the obvious way pays 3 additions each &mdash; 24 in all. The instinct way pays 3 once to get going, then 2 per slide for 7 slides &mdash; 17. A small lead here, but the obvious way&rsquo;s bill grows with the window&rsquo;s width, and the instinct way&rsquo;s never does: a window of 100 makes it about fifty times the work for the same answers.</p>
            <p>And however long the row gets, you now touch each number about twice &mdash; once when it joins the window, once when it leaves. Work that grows in step with the row is written <Term word="O(n)"><code>O(n)</code></Term>; the flat two-steps-per-slide is <Term word="O(1)"><code>O(1)</code></Term>. You counted both before naming them.</p>
            <p>Three corner cases, so nothing bites later: a window wider than the row means there are no windows at all (the code answers with an empty list). A window exactly as wide as the row gives one answer and zero slides. A window of width 1 makes every number its own answer &mdash; the slide still works; there&rsquo;s just nothing left to reuse.</p>
          </>
        ),
        structured: (
          <>
            <p>The ledger: rebuild = 3 additions &times; 8 windows = 24. Running total = 3 once + 2 &times; 7 slides = 17. A small gap at <code>k = 3</code>; enormous at <code>k = 100</code> &mdash; the rebuild scales with <code>k</code>, the running total doesn&rsquo;t.</p>
            <p>Counted, now named: the per-slide update is <Term word="O(1)"><code>O(1)</code></Term> (flat cost at any width), and the whole pass touches each element about twice &mdash; <Term word="O(n)"><code>O(n)</code></Term> for all windows together, versus roughly n &times; k for the rebuild.</p>
            <p>Edges worth a glance: <code>k &gt; n</code> or <code>k &le; 0</code> &mdash; no valid window, and the code&rsquo;s guard line returns an empty list. <code>k = n</code> &mdash; one window, zero slides. <code>k = 1</code> &mdash; the sums are the elements themselves. All of them fall out of the same loop bounds.</p>
          </>
        ),
        rigorous: (
          <>
            <p><strong>Exact cost.</strong> Setup: one k-cell sum. Each of the n &minus; k slides: one subtract, one add &mdash; k + 2(n &minus; k) operations total, against k(n &minus; k + 1) for the rebuild. Asymptotically <Term word="O(n)"><code>O(n)</code></Term> versus O(n·k): the width cancels out of the streaming version. Extra memory beyond the output: O(1).</p>
            <p><strong>Edges.</strong> k &le; 0 or k &gt; n: zero windows &mdash; the guard returns <code>[]</code> before any arithmetic. k = n: one window; the loop never runs. k = 1: each element is its own window; the update degenerates to replacement. Python ints never overflow the running sum; floats drift under repeated &plusmn; &mdash; re-sum periodically if it matters. And the update must be undoable: sums subtract cleanly; max/min don&rsquo;t &mdash; a sliding max needs different machinery.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>That&rsquo;s the name. You&rsquo;ll spot it whenever you look at side-by-side stretches of a row and the answer can be <strong>nudged</strong> as the window moves instead of rebuilt each time. Open the <strong>code</strong> tab on the right for the full Python &mdash; trace one pass.</>,
          ...
          structured: <>That&rsquo;s the name: <strong>sliding window</strong>. The signal: contiguous stretches whose answer updates incrementally as the window moves. File it under idea 1 of 7 &mdash; <strong>information reuse</strong>: pay for each element once, then repair the answer instead of rebuilding it. The <strong>code</strong> tab has the full pass.</>,
          rigorous: <>Sliding window. Trigger: contiguous ranges &mdash; fixed width here &mdash; with an incrementally maintainable aggregate. Idea 1 of 7, <strong>information reuse</strong>: compute each contribution once, repair the aggregate per step. Next door, the variable-width version: the invariant drives the shrink.</>,
        }),
      }],
      detail: reg({
        base: (
        ...
        intuitive: (
          <>
            ...
            <ul>
              <li>&ldquo;every stretch of <code>k</code> in a row&rdquo; &mdash; days, letters, readings</li>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The first big idea (1 of 7) &mdash; information reuse:</strong> pay to figure something out once, then reuse it instead of redoing it. You just used it to beat a whole family of problems.
            </div>
        ...
        rigorous: (
          <>
            <p>Triggers: &ldquo;contiguous subarray / substring of length k&rdquo;; any aggregate over all length-k ranges (sum, mean, count, hash); scans where consecutive ranges differ by one element per edge. Requirement: the aggregate supports removal &mdash; sums and counts do; max/min need a monotonic deque instead.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 1 of 7 &mdash; information reuse:</strong> compute each fact once; repair, don&rsquo;t rebuild. Prefix sums, memoization, DP tables &mdash; the same ledger at scale.
            </div>
