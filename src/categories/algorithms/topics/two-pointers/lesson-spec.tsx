...
export const twoPointersLesson: LessonSpec = {
  ...
  bridgeFrom: reg({
    base: "An array hands you any slot in one step. A sorted array's order can also tell you which slots never to visit.",
  ...
  beats: [
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>Ten number cards lie in a row, smallest to largest. A friend names a total, say <strong>17</strong>, and asks for two cards that add up to it exactly. The question: how few cards must you touch?</>,
          intuitive: <>Ten number cards lie in a row, already in order: smallest on the left, largest on the right. A friend names a total, <strong>17</strong>, and wants two cards that add up to it exactly. The game: be sure of the answer while touching as few cards as you can.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>Picture ten cards laid out in a row, smallest on the left, largest on the right. A friend points at a number, let&rsquo;s say <code>17</code>, and asks: &ldquo;Find me two cards that add up to exactly this.&rdquo;</p>
            <p>You&rsquo;re allowed to pick any two cards, lift them up, check their total, and put them back. The real question isn&rsquo;t <em>can</em> you do it. It&rsquo;s how <strong>few</strong> cards you have to touch before you&rsquo;re sure. That &ldquo;how few&rdquo; is the whole game here.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Start with the table: ten cards in a row, laid left to right from smallest to largest. Your friend points at a number, <code>17</code>, and says: &ldquo;two of these add up to exactly that. Find them.&rdquo;</p>
            <p>The rules are gentle: lift any two cards, total them, put them back. Anyone can find the pair eventually. The real game is being <em>sure</em> while touching as few cards as possible. Keep that score in your head for the whole lesson: how many touches?</p>
          </>
        ),
      }),
      codeLabels: ["sig"],
    },
    {
      ...
      takeaway: "Trying every pair is up to 45 tries (O(n²)), and it ignores that the cards are sorted.",
      visual: <BruteForce />,
      panels: [{
        left: 150, top: 300, width: 580, variant: "main", label: "The obvious thing", title: "Try every pair until one works.",
        body: <>The honest way: pick one card, test it against every other. Nothing sums to 17? Pick the next, test the rest. For ten cards that&rsquo;s up to <strong>45</strong> tries, and you touch cards over and over. The cards are <strong>sorted</strong>, but we haven&rsquo;t used that.</>,
      }],
      detail: (
        <>
          <p>The honest answer: pick the first card, then test it against every other card. If nothing sums to <code>17</code>, pick the second card and test it against every card after it. Then the third, and so on. This is the <strong>brute-force</strong> way: try every possible pair and hope one fits.</p>
          <p>For ten cards that&rsquo;s <code>9 + 8 + 7 + &hellip; + 1 = 45</code> pairs to inspect, and you end up touching the same cards over and over. Scale it up: a thousand cards means roughly half a million pairs. We call this <Term word="O(n²)"><code>O(n&sup2;)</code></Term> work (&ldquo;order n-squared&rdquo;), meaning if the number of cards <code>n</code> doubles, the effort roughly <em>quadruples</em>.</p>
    ...
    },
    {
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Put one finger, call it <Term word="L"><code>L</code></Term>, on the smallest card, one (<Term word="R"><code>R</code></Term>) on the largest. <em><Term word="arr[i]"><code>arr[L]</code></Term> just means the card the L finger points at.</em> Add the two. Use the buttons: when the sum is too small, which finger makes it bigger?</>,
            intuitive: <>One finger, call it <Term word="L"><code>L</code></Term>, goes on the smallest card, the other (<Term word="R"><code>R</code></Term>) on the largest. <em><Term word="arr[i]"><code>arr[L]</code></Term> is just &ldquo;the card under the L finger.&rdquo;</em> Add the two cards. Now play with the buttons: when the total comes out too small, which finger makes it grow?</>,
          }),
        },
        {
      ...
      detail: reg({
        base: (
          <>
            <p>Put your left finger on the smallest card and your right finger on the largest. We&rsquo;ll call them <code>L</code> and <code>R</code>, just names for &ldquo;the card the left finger is on&rdquo; and &ldquo;the card the right finger is on.&rdquo; Add the two numbers together. Then use the buttons to move whichever finger you like, and just <em>watch</em> what the sum does.</p>
            <p>Don&rsquo;t reach for a strategy yet. Notice the feel of it: when does each finger want to move? What does it mean when the total comes out too small? Too big? The answer is hiding in plain sight because the cards are sorted. Moving <code>L</code> right always lands on a bigger card, moving <code>R</code> left always lands on a smaller one.</p>
        ...
        intuitive: (
          <>
            <p>Left finger on the smallest card, right finger on the largest. Call them <code>L</code> and <code>R</code>, nothing fancy, just names for &ldquo;where each finger is.&rdquo; Add the two cards they touch. Then press the buttons and simply <em>watch</em> what the total does.</p>
            <p>No strategy yet. Get a feel for it first. Because the cards are in order, moving <code>L</code> right always lands on a bigger card, and moving <code>R</code> left always lands on a smaller one. So one finger can only push the total up, and the other can only pull it down.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The instinct question:</strong> the total comes out too small. Which finger fixes that? Too big: which one?
            </div>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [
        {
          ...
          body: reg({
            base: <>Too big? <code>L</code> is already the smallest card left, so <em>every</em> pair using <code>arr[R]</code> is too big: never look at it again, step <code>R</code> left. Too small? Mirror it: step <code>L</code> right. One look retires a whole side.</>,
            intuitive: <>The fingers read 1 + 20 = 21, too big. <code>L</code> is already on the smallest card in play, so before anything moves, tap below: how much did that single look actually rule out?</>,
            rigorous: <>Sorted <code>arr</code>, target 17; <code>L</code> and <code>R</code> start at the ends. If <code>arr[L] + arr[R]</code> overshoots, every pair containing <code>arr[R]</code> overshoots too, since <code>arr[L]</code> is its smallest available partner, so <code>R</code> is discarded outright. Undershoot mirrors with <code>L</code>. One comparison, one endpoint gone.</>,
          }),
        },
        {
          left: 600, top: 360, width: 252, variant: "note",
          body: reg({
            base: <><strong className="text-[var(--accent-ink)]">Why it works:</strong> sorted is a promise the cards keep, so each comparison cuts a whole side, not one pair.</>,
      ...
      detail: reg({
        base: (
          <>
            <p>Call the fingers <code>L</code> and <code>R</code>, and look at the total <code>arr[L] + arr[R]</code>: the left card plus the right card.</p>
            <p><strong>Too small?</strong> Remember <code>R</code> is sitting on the biggest card left in play. So pairing <code>arr[L]</code> with <em>anything</em> between <code>L</code> and <code>R</code> can only be smaller still. Every one of those pairs is also too small. That means <code>arr[L]</code> can never be part of the answer, so we throw it away for good and step <code>L</code> one card to the right.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The principle:</strong>{" "}being sorted is a promise the cards keep. Each single comparison lets us eliminate a whole side of the remaining cards at once, not just one pair.
            </div>
          </>
        ),
        intuitive: (
          <>
            <p>Look at what the fingers are touching: <code>L</code> is on the smallest card still in play, <code>R</code> on the biggest. Their total came out too big: 21, over 17.</p>
            ...
            <p>Too small is the mirror image: then the card under <code>L</code> is the hopeless one. Even the biggest partner left couldn&rsquo;t lift it to 17, so <code>L</code> steps right instead.</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>The principle:</strong>{" "}sorted order is a promise the cards keep, and that promise lets one look throw away a whole side of pairs, never just one.
            </div>
          </>
        ),
        rigorous: (
          <>
            ...
            <p>Sortedness does all the work: it is the monotone guarantee that makes the discard safe. Unsorted input voids the argument; sort first, or use a hash map instead.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      takeaway: reg({
        ...
        rigorous: "L only rises, R only falls: at most n − 1 comparisons, giving O(n) time, O(1) extra space.",
      }),
      visual: <WinStat />,
      panels: [{
        left: 150, top: 26, width: 560, variant: "main", label: "The win", title: "Forty-five pairs becomes nine comparisons.",
        body: reg({
          base: <>Trying every pair takes up to <code>n &times; (n &minus; 1) / 2</code>. With <code>n</code> (the card count) = 10, that&rsquo;s 45. Two fingers touch each card once: <code>L</code> only moves right, <code>R</code> only left. They meet in at most <code>n &minus; 1</code> = 9 steps.</>,
          intuitive: <>Count it on the cards, not with a formula: every look retires one card for good, and the game ends when the fingers meet. Ten cards, two already under the fingers, so at most <strong>nine</strong> looks. The try-every-pair way needed up to <strong>45</strong>.</>,
          rigorous: <>Each iteration moves exactly one pointer inward, so <code>R &minus; L</code> drops by one per comparison: at most <code>n &minus; 1</code> comparisons, against brute force&rsquo;s <code>n(n &minus; 1)/2</code> pair inspections. That linear count is <Term word="O(n)"><code>O(n)</code></Term> time, and <Term word="O(1)"><code>O(1)</code></Term> extra space: two indices, nothing else.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>Put the two costs side by side. Brute force inspects up to <code>n &times; (n &minus; 1) / 2</code> pairs. Here <code>n</code> is the card count, so with <code>n = 10</code> that&rsquo;s 45 comparisons. That formula is <Term word="O(n²)"><code>O(n&sup2;)</code></Term> work: double the cards and the effort roughly quadruples.</p>
            <p>The two fingers, by contrast, touch each card at most once. <code>L</code> only ever moves right, <code>R</code> only ever moves left, and they march toward each other until they meet. So after at most <code>n &minus; 1</code> = 9 steps the search is over. That&rsquo;s <Term word="O(n)"><code>O(n)</code></Term> work (&ldquo;order n&rdquo;): the effort grows in simple step with the number of cards, not with its square.</p>
            <p>Forty-five collapses to nine, and the gap only widens as the row gets longer. Next, see the fingers actually converge.</p>
            <p>Edge cases, while we&rsquo;re counting: if <em>no</em> pair sums to 17, the fingers simply meet and the search ends honestly, returning <code>None</code> after those same at-most-9 looks. The loop&rsquo;s <code>L &lt; R</code> condition also means a card never pairs with itself, and an empty or one-card row ends before a single look.</p>
          </>
        ),
        intuitive: (
          <>
            <p>Line the two costs up. The try-every-pair way: up to 45 looks for ten cards, and double the cards means roughly four times the looks. Growth like that is what people write as <Term word="O(n²)"><code>O(n&sup2;)</code></Term>.</p>
            <p>The two fingers: every look retires a card for good. <code>L</code> never backs up, <code>R</code> never backs up, and the game stops when they meet, so ten cards cost at most nine looks. Counting like that, effort growing in step with the cards, is what <Term word="O(n)"><code>O(n)</code></Term> means.</p>
            <p>And when there is <em>no</em> winning pair at all? Nothing breaks: the fingers meet, the answer is &ldquo;no pair,&rdquo; and it still took at most nine looks. A row with one card (or none) ends before a single look, since the fingers have nowhere to stand. And a card never gets paired with itself, because the two fingers must sit on different cards.</p>
          </>
        ),
        rigorous: (
          <>
            <p><strong>Exact cost.</strong> Every iteration does constant work and moves exactly one pointer inward, so the window width <code>R &minus; L</code> falls from <code>n &minus; 1</code> to 0: at most <code>n &minus; 1</code> iterations, best case 1. Time <Term word="O(n)"><code>O(n)</code></Term>, extra space <Term word="O(1)"><code>O(1)</code></Term>, versus <code>n(n &minus; 1)/2</code> pair inspections for brute force. (Input not sorted? Sorting first costs <code>O(n log n)</code> and dominates.)</p>
            <p><strong>Edge cases.</strong> No valid pair: the pointers meet and the function returns <code>None</code>, so a miss costs the same O(n). Empty or single-element input: <code>left &lt; right</code> is false immediately, zero iterations. The strict <code>&lt;</code> also forbids pairing an element with itself. Duplicates are harmless: the first valid pair found is returned. Precondition: ascending order; unsorted input silently breaks the discard argument.</p>
          </>
        ),
      }),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: <>The two fingers start at the ends and step inward by themselves, each move shrinking what&rsquo;s left. Faded cards are retired, out of play for good. When the sum lands on 17, the pair turns green. Use <strong>&ldquo;↺ replay&rdquo;</strong> under the visual to run it again. This board finishes in well under nine looks.</>,
      }],
      detail: (
        <>
          <p>The rule from the last two beats now runs on its own. Just watch. The two fingers start at the far ends and step inward one move at a time, and every move shrinks the part of the row still in play.</p>
          <p>The faded cards aren&rsquo;t just dimmed for looks. They&rsquo;re the ones we <em>retired</em>, gone for good because we proved they couldn&rsquo;t be part of the answer. When <code>arr[L] + arr[R]</code> finally lands on <code>17</code>, that pair turns green and the search stops. Count the moves: this board reaches the answer in well under the nine looks the worst case allows, because the fingers often meet the target before they ever get close to each other.</p>
        </>
      ),
      codeLabels: ["loop", "compute", "compare", "found"],
    ...
    },
    {
      ...
      detail: (
        <>
          ...
          <p>The fingers genuinely don&rsquo;t care <em>what</em> they&rsquo;re comparing. All they need is that the thing underneath has a <em>direction</em> (sorted order, symmetry, height) that tells each finger which way to step.</p>
        </>
      ),
    ...
    },
    {
      ...
      panels: [{
        ...
        body: reg({
          base: <>That&rsquo;s the name. Reach for it whenever a row has a <strong>direction</strong> (sorted order, symmetry, a one-way pattern) and one comparison can retire a whole side. Signals: &ldquo;sorted array + pair sum&rdquo;; &ldquo;palindrome&rdquo;; &ldquo;container with most water.&rdquo;</>,
          intuitive: <>Its name: <strong>Two Pointers</strong>, where the &ldquo;pointers&rdquo; are your two fingers. Reach for it when a row has a <strong>direction</strong> (sorted order, symmetry, a one-way pattern) so one look can retire a whole side. And file the why: idea 3 of 7, <strong>keep a guarantee that lets you skip work</strong>.</>,
          rigorous: <>Two Pointers. It applies when the data carries a monotone guarantee (sorted order, symmetry, a one-way bound) strong enough that one comparison soundly discards a whole side. That is idea 3 of 7, <strong>monotonicity and invariants</strong>: maintain a guarantee that lets you skip work.</>,
        }),
      }],
      detail: reg({
        base: (
          <>
            <p>That&rsquo;s the name: <strong>Two Pointers</strong>. The &ldquo;<Term word="pointer">pointers</Term>&rdquo; are just the two fingers, markers that each remember a position in the row. The pattern fits whenever a line of data has a <em>direction</em> (sorted order, symmetry, or some one-way pattern) and a single comparison can rule out a whole side of what&rsquo;s left, instead of just one item.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 3 of 7, monotonicity and invariants:</strong> maintain a guarantee that lets you skip work. Here the guarantee was sorted order, and it paid out a whole side of skipped pairs per look.
            </div>
        ...
        intuitive: (
          <>
            <p>The move has a proper name: <strong>Two Pointers</strong>. The &ldquo;<Term word="pointer">pointers</Term>&rdquo; are just the two fingers, markers that each remember a spot in the row. It fits whenever the row has a <em>direction</em> (sorted order, symmetry, some one-way pattern) so that one look can rule out a whole side, never just one card.</p>
            ...
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 3 of 7, keep a guarantee that lets you skip work:</strong> the sorted order was a promise the cards kept, and every look cashed that promise in for a whole side of skipped pairs.
            </div>
        ...
        rigorous: (
          <>
            <p><strong>Two Pointers.</strong> Preconditions: a monotone structure (sorted order, symmetry, a one-way bound) and a comparison that classifies one endpoint as disposable. Mechanics: two indices move monotonically toward each other; the invariant, that any answer lies within the window, is preserved per step; termination in &le; n &minus; 1 steps. Variants: pair-sum on sorted input, palindrome check (&asymp; n/2 steps), container-with-most-water (advance the shorter line), in-place dedup (read/write pointers).</p>
            <div className="mt-1 p-3 rounded-lg bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--text)]">
              <strong>Idea 3 of 7, monotonicity and invariants:</strong> maintain a guarantee that lets you skip work; here sortedness pays out one soundly discarded endpoint per comparison.
            </div>
          </>
        ),
      }),
