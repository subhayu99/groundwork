...
- Phase 1 (2026-05-31): contrast→AA (globals.css), mobile header grid + accessible stepper + canvas role=img/aria-live + label de-dup + practice `<a>`→`<Link>` + prereq prop (LessonRuntime), focus rings + state-in-aria (canvas.tsx), new PrereqNudge.tsx, prereq wiring (TopicPageClient), home "Start with Binary Search" CTA (page.tsx), concept-map prereq edges + ready/start-here styling + legend (ConceptMapHome). tsc clean, build 95 pages, verified desktop+mobile. SHA `a51ba3b`.
- Phase 5 — IMMERSION (2026-06-01): built the one-scene layout (`SceneLayout.tsx` + `useLessonEngine` hook + `LessonSpec.layout`/`LessonBeat.takeaway`), gated by `layout:"scene"`. Piloted on binary-search (`fcf0c4b`) and refined over: rough edges (`67bb849`), code-drawer occlusion + control bar (`3aeaa3e`), consistent caption band (`092b0a3`), one-place code/card toggles + Focus keeps card (`2b6b208`), centered-but-pinned rail (`05ed578`), contextual card header (`0f649b0`), sticky card state (`fb3b2ff`), card-top↔caption-band alignment (`8fcb6ed`). **Fanned out to all 20 topics + fixed widthFill clip: `83627c0`.** All 20: tsc clean, build 95 pages, overlap 0, no clipping. Remaining Phase 5 (optional): per-topic climax 7a/7b split, mobile pinned active-code-line strip.
